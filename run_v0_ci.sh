#!/usr/bin/env bash
# Mirror of run_v1_ci.sh but with SGLANG_OMNI_SERVER_VERSION=legacy (V0).
# Writes to a separate log_CI_v0.txt so V1 logs are not touched.
...
export SGLANG_OMNI_SERVER_VERSION=legacy
export SGLANG_SEEDTTS50_DIR=/data/chenyang/sglang-omni/.benchmark-data/seedtts50
export CUDA_VISIBLE_DEVICES=1,5

LOG=/data/chenyang/sglang-omni/log_CI_v0.txt
SUMMARY=/data/chenyang/sglang-omni/log_CI_v0_summary.txt
: > "$LOG"
: > "$SUMMARY"

{
  echo "=== V0 (legacy) CI run started at $(date -Iseconds) ==="
