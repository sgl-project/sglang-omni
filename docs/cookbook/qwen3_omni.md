...
For native Torch MPS INT4, use a separate HF-native quantized checkpoint:

```bash
env -u SGLANG_USE_MLX -u SGLANG_QWEN3_OMNI_MPS_QUANTIZATION \
  PYTORCH_ENABLE_MPS_FALLBACK=0 \
  "$PY" -m sglang_omni.cli serve \
  --model-path /absolute/path/to/Qwen3-Omni-AutoRound \
  --model-name qwen3-omni \
  --host 127.0.0.1 \
  --port 8008
```

Use a separate HF-native checkpoint, not the MLX `MODEL_DIR` above.
The [usage guide](../basic_usage/qwen3_omni.md#apple-silicon-mlx-and-torch-mps)
lists pinned Intel AutoRound and Cyankiwi compressed-tensors exports and download
commands. Their Thinker weights stay packed; the Talker and other excluded
components use the ordinary dense loaders. There is no online quantization, MLX
conversion, INT8 conversion, or CUDA fallback on this path. The normal MPS
launch automatically selects dense or supported INT4 loading from checkpoint
metadata; the old `int4` environment flag is optional. The eager serving profile is
unchanged; loading a recognized format alone does not establish full-model
memory safety or semantic production qualification.
