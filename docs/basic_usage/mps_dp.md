...
| Qwen3-Omni (`Qwen3OmniThinkerForCausalLM` + `Qwen3OmniTalker`) | Audited (engine-level gate only; the mps_dp launcher cannot drive a two-engine pipeline yet) | both engines' registered tensors (towers and code2wav stay per-replica stages) | none registered (talker staging is unregistered plain tensors) | mutation audit of both engines + shared protocol tests; DP launches need manual per-stage supervision with explicit follower caps on both engines |
