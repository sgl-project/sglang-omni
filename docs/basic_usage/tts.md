...
This guide uses [Fish Speech S2-Pro](https://huggingface.co/fishaudio/s2-pro) as an example TTS (text-to-speech) model with SGLang-Omni and the OpenAI-compatible API. The same `/v1/audio/speech` endpoint also supports Voxtral TTS, Qwen3-TTS, MOSS-TTS, and CosyVoice3.
...
| [CosyVoice3](../cookbook/cosyvoice3.md) | `examples/configs/cosyvoice3.yaml` | Zero-shot voice cloning, single-utterance, 24 kHz. Requires a reference clip via `references[0].audio_path` (+ `text`). Reads local wavs with `--allowed-local-media-path` |
...
For CosyVoice3 (the model path is baked into the config; reference wavs are read from a local
directory):
...
Qwen3-TTS CustomVoice, Voxtral, and S2-Pro. It is not valid for Qwen3-TTS Base or
CosyVoice3, which both require reference audio (CosyVoice3 clones zero-shot from it).
