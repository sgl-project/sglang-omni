...
sgl-omni serve --model-path /path/to/Ming-omni-tts-16.8B-A3B \
  ...
  --host 0.0.0.0 --port 8000
# synthesize (the checkpoint ships its own speaker):
curl -s -X POST http://localhost:8000/v1/audio/speech \
  -H "Content-Type: application/json" \
...
The streaming AudioVAE graph resolves to eager on XPU, so
`examples/configs/ming_omni_tts.yaml` serves as shipped even though it sets
`streaming_cuda_graph: true` for CUDA. That graph decodes with `torch.fft.irfft`, which
oneMKL implements by waiting on an event a command graph cannot own, so the audio decode
stage logs `resolved=eager` and runs the eager streaming vocoder, correct on XPU.
