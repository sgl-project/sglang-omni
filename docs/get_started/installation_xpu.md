...
Models that reach sgl-kernel's SYCL JIT kernels — Ming-Omni-TTS does, for joint RoPE — need `icpx`
on `PATH` at serve time. The check runs before the on-disk cache and the compiler version is part of
the cache key, so a warm cache does not remove the requirement. Add the compiler directory alone
rather than sourcing `setvars.sh`, which would reintroduce the oneCCL conflict above:
```bash
export PATH="/opt/intel/oneapi/compiler/latest/bin:$PATH"   # icpx only, no oneAPI libraries
```
Without it the stage fails at capture with
`RuntimeError: icpx compiler not found. Please install Intel oneAPI toolkit and source setvars.sh`.
...
Leave the streaming AudioVAE graph off on XPU. `examples/configs/ming_omni_tts.yaml`
sets `streaming_cuda_graph: true` for CUDA, and that graph decodes with
`torch.fft.irfft`, which oneMKL implements by waiting on an event a command graph
cannot own (`wait method cannot be used for an event associated with a command
graph`). Pass `--audio_decode.factory.streaming_cuda_graph false` when serving with
that config; the eager streaming vocoder is correct on XPU.
