...
## 第三十七轮:独立进程的 vocoder 开 MPS,rps 20 首帧损失收回一半(2026-09-23 18:29-19:28 PT)

Tilde 单卡(worker-3,H100 80GB HBM3),main `a0087317`,Slurm 作业 379731,四个臂依次跑,各 3 个 seed。

| rps 20 | D0d1 出厂默认(同进程) | D1Ma 独立进程 + `mps: "on"` | D1rb 独立进程、无 MPS |
|---|---|---|---|
| 首帧 p50(ms) | 41.4(41.2 到 42.0) | **47.9(47.0 到 49.1)** | 55.5(53.8 到 56.0) |
| 首帧 p95 | 68.8 | 84.1 | 91.4 |
| 可闻 p50 | 42.9 | **55.6** | 68.8 |
| 可闻 p95 | 87.1 | 131.4 | 141.2 |
| underrun | 0 到 0.2% | 1.4 到 4.6% | 3.1 到 7.4% |

rps 1:D0d1 29.9、D1Ma 30.3、D1rb 32.3 ms。第四个臂 D1Mb(MPS 复测)没起来:harness 杀掉 D1Ma 的进程树时没清 MPS 的状态目录,
下一次启动按设计拒绝在脏状态上起 daemon(`MpsError: MPS state dir ...`),是我的 harness 问题,不是产品缺陷。

**读法**:两个对照臂与第三十五轮一致(D0 41.4 对 40.8 到 41.9,独立进程 55.5 对 56.3 到 59.0),读数可信。
MPS 把独立进程在 rps 20 的首帧损失从 14.1 ms 收回到 6.5 ms(收回约 54%),可闻首帧从 +25.9 收回到 +12.7 ms,
rps 1 则完全收回(30.3 对 29.9)。但 MPS 下仍有 1.4 到 4.6% 的 underrun,同进程是 0 到 0.2%。
只有一个 MPS 臂(3 个 seed,区间与无 MPS 臂不重叠),没有复测块,这是本轮的局限。

**含义**:单卡部署 CustomVoice 时,同进程(出厂默认)首帧与卡顿都最好;必须拆独立进程时应开 MPS,能收回一半。
CI 与 cookbook 基准表用的是独立进程、不开 MPS 的形态,它的首帧比出厂默认多 14 ms。CI preset 的注释说拆出 vocoder 是为了测
"调好的点",可能是吞吐上的考虑,所以不能直接改 CI 形态;下一步把这组数与吞吐对照一起拿给 preset 的作者(JiaxinD)讨论,
再决定 cookbook 的推荐形态。已写进 #2294 正文。

**CI 主机上的第一次验证失败了(2026-09-23 21:26 PT)**:#2293 在 `4606c6b9` 的 CI 按 `run-qwen3-tts` 跑了 Base 臂
(`omni-runner-h100-3`,`host-85-234-79-221`,lane `16-31,80-95`):1 rps 中位数 57.9 ms(这里是 55.7 到 60.0),
20 rps 中位数 122.1 ms(这里 106.0 到 117.5,gate 139.2 通过),**20 rps p95 256.4 ms,超过 gate 236.8,判失败**(这里 173.9 到 194.2)。
lane 的核上没有外来负载(`[cpuset-contention]` 均值 0.06 核),但同一 PR 的 stage 4 在 04:24:41Z 于另一 lane 起服务,
正好压在 20 rps 那一段上。也就是说 CustomVoice 在 CI 主机上更快,Base 在负载下反而更慢、尾部重得多。
外审第一条("参照要取自 CI 主机重跑""不能把 CustomVoice 的主机差迁移到 Base")说中了,我当时以"CI 主机更快、只会偏松"为由没采纳,
这个理由只对 CustomVoice 成立。

**改判(决策四件套)**
- 问题:gate 在 CI 主机上误报,怎么办。
- 默认:照外审的建议改用 CI 主机的观测做参照。等这次 CI 跑完(现在推送或重跑会通过并发组取消还在跑的 stage 4),
  重跑延迟 job 攒观测:Base 在当前提交重跑(gate 可能再失败,但日志里有完整指标);CustomVoice 重跑旧 run `35921567519`
  (`4a8cef3d`,该臂当时只打印不判,且与现在只差阈值数字,测量等价)。每臂凑满 5 次 CI 主机观测后,参照 = CI 主机 worst,
  阈值规则不变(× 1.25),eval-h100 的数据保留为第二环境的证据,不与 CI 主机混在一起。
- 理由:CI 的真实环境包括其他 lane 同时跑的 job,这台机器上 Base 的尾部对它很敏感;只有 CI 主机的观测能代表 gate 要判的东西。
- 回滚:参照只是 `_QWEN3_TTS_*_LATENCY_REFERENCE` 里的数字;若 CI 主机上的波动大到 × 1.25 也压不住,退回只打印(`calibrated=False`),
  并在 PR 里写明原因。

**改用 CI runner 的观测后(2026-09-24 00:10 PT,#2293 `36bcafe2`)**:在 PR 上把延迟 job 原样重跑,每臂凑满 5 次
(Base 在 `4606c6b9`,CustomVoice 在 `4a8cef3d` 上只打印;两者与最终提交只差阈值数字)。每次重跑会把被取消的 Omni 阶段一起带起来,
脚本在延迟 job 结束后立刻取消其余部分,不占多余的 CI 时间。落在三条 lane 上(`16-31,80-95`、`48-63,112-127`、`32-47,96-111`),
其余 lane 上同时跑着别的 PR 的 job。首帧,ms(中位数 / p95):

| 臂 | 1 rps | 20 rps |
|---|---|---|
| Base | 57.9 / 82.0,58.1 / 80.3,59.6 / 85.0,57.3 / 75.0,58.9 / 77.2 | 122.1 / 256.4,103.9 / 172.4,105.5 / 182.4,105.7 / 173.1,112.9 / 308.2 |
| CustomVoice | 20.2 / 27.2,20.5 / 28.1,21.5 / 27.8,20.0 / 25.7,22.3 / 29.8 | 34.8 / 49.2,33.2 / 59.0,35.3 / 48.9,35.2 / 56.7,33.0 / 49.5 |

- 新参照(CI runner worst)与 gate:Base 1 rps 59.6 → 74.5、20 rps 122.1 → 152.6;CustomVoice 22.3 → 27.9、35.3 → 44.1、20 rps p95 59.0 → 73.7。
- **Base 20 rps p95 不 gate、只打印**:五次里两次是 256 与 308,另外三次 172 到 182,是双峰;同一批运行的中位数只差 17%。
  按校准手册"延迟类相对极差超过约 20% 到 30% 就是噪声阶段、先标记"的说法,这一项在 runner 上是噪声阶段。
  查过两次重尾运行时同机其他 lane 在跑什么:都有 TTS 或 Omni 的重 job,但正常的几次同样有,所以说不清具体是哪类负载拖尾。
- 按手册的破坏性轮次规则,第 5 次(p95 308.2,与最近邻 256.4 差 20.2%,MAD z 8.5)会被判破坏性;但它不影响任何被 gate 的参照
