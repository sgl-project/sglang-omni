...
async def main_async(args: argparse.Namespace) -> dict[str, Any]:
    ...
    async with aiohttp.ClientSession(timeout=timeout, connector=connector) as session:
        ...
        memory_checkpoints.append(
            _memory_checkpoint("after_cooldown", args.gpu_index)
        )
...
async def _run_functional_checks(
    ...
    near_limit = _resize_wav(en_sample.audio_bytes, 29.9)
    ...
    checks.append(_expect_success("near_30s_audio", near_result))

    over_limit = _resize_wav(en_sample.audio_bytes, 30.1)
    checks.append(
        _expect_status(
            "over_30s_audio",
            await _post_raw_audio(
