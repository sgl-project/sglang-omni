...
"""WER evaluation for Qwen3-Omni TTS generated audio.

Uses Qwen3-Omni-30B-A3B-Instruct with HuggingFace transformers to generate
speech, then evaluates Word Error Rate using Whisper (English) or FunASR
(Chinese).

Usage:
    # English
    python -m benchmarks.performance.tts.benchmark_tts_wer_qwen3_omni \
    ...
    python -m benchmarks.performance.tts.benchmark_tts_wer_qwen3_omni \
        ...
        --model-path Qwen/Qwen3-Omni-30B-A3B-Instruct \
        --output-dir results/qwen3_omni_zh \
        --lang zh
"""
...
# ---------------------------------------------------------------------------
# Qwen3-Omni speech generation
# ---------------------------------------------------------------------------


def generate_speech_qwen3_omni(
    model: Qwen3OmniMoeForConditionalGeneration,
    processor: Qwen3OmniMoeProcessor,
    sample: SampleInput,
    speaker: str = "Ethan",
) -> tuple[np.ndarray, int, float]:
    """Generate speech using Qwen3-Omni via HuggingFace transformers."""
    conversation = [
        {
            "role": "user",
            "content": [
    ...
    if audio_wav is None:
        raise ValueError(
            f"Model returned no audio for sample '{sample.sample_id}'. "
            f"text_ids sequences shape: {text_ids.sequences.shape}"
        )

    audio_numpy = audio_wav.reshape(-1).detach().cpu().numpy()
    return audio_numpy, QWEN3_OMNI_SAMPLE_RATE, latency
...
def evaluate_sample(
    ...
    # Generate audio
    try:
        audio_data, sr, latency = generate_speech_qwen3_omni(
            model, processor, sample, speaker
        )
...
def main() -> None:
    p = argparse.ArgumentParser(
        description="WER evaluation for Qwen3-Omni TTS (Whisper for EN, FunASR for ZH)"
    )
    ...
    p.add_argument(
        "--device",
        default="auto",
        help="device_map for Qwen3-Omni model (default: auto)",
    )
    p.add_argument(
        "--asr-device",
        default="cuda:0",
    ...
    # Load ASR model on a dedicated device to avoid OOM with Qwen3-Omni
    asr = load_asr_model(args.lang, args.asr_device)

    # Load Qwen3-Omni
    logger.info("Loading Qwen3-Omni from %s (device_map=%s)...", args.model_path, args.device)
    t0 = time.perf_counter()
    ...
    qwen_processor = Qwen3OmniMoeProcessor.from_pretrained(args.model_path)
    logger.info("Qwen3-Omni loaded in %.1fs", time.perf_counter() - t0)
