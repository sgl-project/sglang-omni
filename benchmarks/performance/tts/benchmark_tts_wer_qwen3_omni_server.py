...
"""WER evaluation for Qwen3-Omni TTS via sglang-omni server.

Sends TTS requests to a running sglang-omni server using the
/v1/chat/completions endpoint with modalities: ["text", "audio"],
then evaluates Word Error Rate using Whisper (English) or FunASR (Chinese).

Dataset:

    Download from Hugging Face:

        # full set (5k samples: en + zh + hardcases)
        huggingface-cli download zhaochenyang20/seed-tts-eval \
            --repo-type dataset --local-dir seedtts_testset

    Or use --download-dataset to auto-download before evaluation.

Server:

    # Start Qwen3 Omni speech server (requires 2 GPUs):
...
Usage:
    ...
    # English (first 50 samples)
    python -m benchmarks.performance.tts.benchmark_tts_wer_qwen3_omni_server \
        --meta seedtts_testset/en/meta.lst \
        --output-dir results/qwen3_omni_server_en_50 \
...
async def generate_speech_server(
    ...
    if lang == "en":
        ...
        prompt_text = (
            f"请用中文朗读以下文本: {sample.target_text}"
        )
...
def print_summary(metrics: dict, args: argparse.Namespace) -> None:
    ...
    print(f"\n{'=' * w}")
    print(f"{'Qwen3-Omni Server TTS WER Result':^{w}}")
    print(f"{'=' * w}")
