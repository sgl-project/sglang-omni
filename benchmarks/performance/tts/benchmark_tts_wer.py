...
Dataset: seed-tts-eval
    ...
    The dataset is auto-downloaded from Hugging Face by default.
    You can also provide a local path via --meta.
...
Usage:
    ...
    # Auto-download dataset from HF and evaluate English:
...
def download_dataset(
    ...
    """Download the seed-tts-eval dataset from HuggingFace and return meta.lst path.

    Only downloads files for the requested language to save bandwidth.
    """
    from huggingface_hub import snapshot_download

    meta_path = os.path.join(local_dir, lang, "meta.lst")
    if os.path.isfile(meta_path):
        logger.info("Dataset already exists at %s, skipping download", meta_path)
        return meta_path

    logger.info("Downloading dataset %s (lang=%s) to %s ...", repo_id, lang, local_dir)
    snapshot_download(
        repo_id,
        repo_type="dataset",
        local_dir=local_dir,
        allow_patterns=[f"{lang}/**"],
    )
    if not os.path.isfile(meta_path):
        raise FileNotFoundError(
            f"Downloaded dataset but {meta_path} not found. "
            f"Check that {repo_id} contains {lang}/meta.lst."
        )
    logger.info("Dataset ready: %s", meta_path)
    return meta_path
...
def resolve_meta_path(args: argparse.Namespace) -> str:
    """Resolve the meta.lst path: use --meta if provided, otherwise download from HF."""
    if args.meta:
        return args.meta
    return download_dataset(args.dataset, args.lang, args.dataset_dir)
...
async def main_async(args: argparse.Namespace):
    ...
    # Resolve dataset path (download from HF if needed)
    meta_path = resolve_meta_path(args)
    args.meta = meta_path  # store resolved path for save_results
    ...
    # Parse samples
    samples = parse_meta_lst(meta_path, args.max_samples)
    logger.info("Loaded %d samples from %s", len(samples), meta_path)
...
def main():
    ...
    p.add_argument(
        "--meta",
        default=None,
        help="Path to a local meta.lst file. If not provided, the dataset "
        "is auto-downloaded from --dataset.",
    )
    p.add_argument(
        "--dataset",
        default=DEFAULT_DATASET_REPO,
        help=f"HuggingFace dataset repo to download (default: {DEFAULT_DATASET_REPO}). "
        "Ignored when --meta is provided.",
    )
    p.add_argument(
        "--dataset-dir",
        default=DEFAULT_DATASET_DIR,
        help=f"Local directory to download the dataset into (default: {DEFAULT_DATASET_DIR}).",
    )
    p.add_argument(
        "--output-dir",
        required=True,
