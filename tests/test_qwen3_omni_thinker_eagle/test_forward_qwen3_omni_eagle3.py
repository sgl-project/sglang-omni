...
def test_forward():
    ...
    target_model = Qwen3OmniMoeThinkerTextModel(
        config=text_cfg,
        quant_config=None,
    ).to(device).to(torch.float16)
    
    # 2. Initialize Draft Model
    draft_model = Qwen3OmniEagle3DraftModel(
        config=text_cfg,
        quant_config=None,
    ).to(device).to(torch.float16)
    ...
    try:
        for step in range(max_gen_steps):
            ...
            if target_hidden is None:
                ...
                target_hidden = torch.randn(batch_size, text_cfg.hidden_size, device=device, dtype=torch.float16)
