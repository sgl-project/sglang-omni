...
"""Tests for the v1 OpenAI-compatible adapter."""
...
from sglang_omni_v1.serve.openai_api import (
    _build_chat_generate_request,
    _build_speech_generate_request,
    _select_speech_audio_delta,
)
from sglang_omni_v1.serve.protocol import ChatCompletionRequest, CreateSpeechRequest
...
class DummyClient:
    ...
    async def speech(
        ...
        *,
        ...
        from sglang_omni_v1.client.audio import encode_audio

        audio_bytes, mime_type = encode_audio(
            audio_data, response_format="wav", sample_rate=sample_rate or 24000
        )
        return SpeechResult(audio_bytes=audio_bytes, mime_type=mime_type, format="wav")
...
def test_build_chat_request_preserves_v1_talker_params() -> None:
...
def test_build_speech_request_preserves_stream_flag() -> None:
    req = CreateSpeechRequest(model="s2-pro", input="hello", stream=True)
...
def test_speech_stream_uses_actual_audio_format_on_fallback(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "sglang_omni_v1.serve.openai_api.encode_audio", _fake_encode_audio
    )
