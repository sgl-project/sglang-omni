...
def yarnless_config() -> SimpleNamespace:
    return SimpleNamespace(rope_scaling=None, rope_parameters=None)


def fake_attn(**overrides) -> SimpleNamespace:
    attrs = dict(
        apply_qk_norm_rope=lambda *a: None,
        head_dim=128,
        config=yarnless_config(),
...
def pin_xpu(monkeypatch: pytest.MonkeyPatch) -> None:
...
def extend_batch(*, mm_inputs) -> SimpleNamespace:
...
def decode_batch() -> SimpleNamespace:
    return SimpleNamespace(
        mm_inputs=None,
...
def make_positions(rows_equal: bool, tokens: int = 6) -> torch.Tensor:
...
def test_equal_rows_make_mrope_collapse_to_the_temporal_row() -> None:
    ...
    assert torch.equal(apply_interleaved_rope(cos, MROPE_SECTION), cos[0])

    diverged = torch.randn(3, 6, 64)
    assert not torch.equal(apply_interleaved_rope(diverged, MROPE_SECTION), diverged[0])
...
def test_upstream_builds_identical_rows_for_a_text_only_batch(
    ...
    ForwardBatch._compute_mrope_positions_extend(  # noqa: leading-underscore  # upstream name
...
def test_gate_admits_a_text_only_batch_and_hands_over_one_row() -> None:
    ...
    positions = make_positions(rows_equal=True)

    gate.evaluate(positions, extend_batch(mm_inputs=None))
...
def test_gate_refuses_any_batch_carrying_multimodal_input(mm_inputs: list) -> None:
    ...
    gate.evaluate(make_positions(rows_equal=False), extend_batch(mm_inputs=mm_inputs))
...
def test_installing_twice_leaves_one_wrapper_and_no_recursion(
    ...
    attn = fake_attn(apply_qk_norm_rope=original)
...
def test_the_installed_wrapper_hands_the_kernel_views_of_the_packed_qkv(
    ...
    positions = make_positions(rows_equal=True, tokens=tokens)
    gate.evaluate(positions, extend_batch(mm_inputs=None))
    ...
    assert (
        attn._used_fused_qk_norm_rope_last_call is True
    )  # noqa: leading-underscore  # production name

    gate.evaluate(positions, decode_batch())
...
def test_gate_refuses_decode_so_no_graph_can_freeze_the_decision() -> None:
    ...
    gate.evaluate(make_positions(rows_equal=True), decode_batch())
...
def test_gate_refuses_when_positions_are_not_mrope() -> None:
    ...
    gate.evaluate(torch.arange(4), extend_batch(mm_inputs=None))
...
def test_install_is_refused_while_a_prefill_graph_could_freeze_it(
    ...
    pin_xpu(monkeypatch)
    layer = SimpleNamespace(self_attn=fake_attn())
