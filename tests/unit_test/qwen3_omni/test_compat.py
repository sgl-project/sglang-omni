...
from sglang_omni.models.qwen3_omni.stages import _apply_compat_overrides
...
def _patch_torch_caps(*caps):
    ...
    fake.cuda.get_device_capability.side_effect = lambda i: caps[i]
    return m.patch.dict("sys.modules", {"torch": fake})


def test_sm80_host_injects_triton():
    with _patch_torch_caps((8, 0)):
