...
def reference(wav, frame_length, sample_rate, window=WINDOW):
    return kaldi.fbank(
        wav,
        num_mel_bins=N_MELS,
...
def make_wav(seconds, sample_rate, seed=0):
...
def test_matches_kaldi_fbank_bit_exactly(seconds, sample_rate):
    wav = make_wav(seconds, sample_rate)
    frame_length = min(25.0, seconds * 1000)
    expected = reference(wav, frame_length, sample_rate)
    got = cached_fbank(
        wav,
        num_mel_bins=N_MELS,
        frame_length=frame_length,
...
def test_short_audio_caps_frame_length_like_the_caller():
    ...
    sample_rate = 16000
    seconds = 0.012
    wav = make_wav(seconds, sample_rate, seed=3)
    frame_length = min(25.0, seconds * 1000)
    expected = reference(wav, frame_length, sample_rate)
    got = cached_fbank(
        wav,
...
def test_matches_for_each_window_type(window):
    ...
    wav = make_wav(1.0, 16000, seed=7)
    expected = reference(wav, 25.0, 16000, window=window)
    got = cached_fbank(
        wav,
        num_mel_bins=N_MELS,
        frame_length=25.0,
