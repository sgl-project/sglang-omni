...
def test_mixed_fraction_and_byte_stages_sum_in_their_own_domains() -> None:
    config = PipelineConfig(
        model_path="dummy",
        stages=[
            make_stage("preprocess", gpu=0, fraction=0.10, next_stage="thinker"),
            make_stage(
                "thinker",
                gpu=0,
                kv_cache_bytes=2 * 1024**3,
...
def test_placement_summary_includes_kv_cache_bytes(monkeypatch) -> None:
    ...
    config = PipelineConfig(
        ...
        stages=[
            make_stage("preprocess", gpu=0, fraction=0.10, next_stage="thinker"),
            make_stage("thinker", gpu=0, kv_cache_bytes=2 * 1024**3, terminal=True),
        ],
    )
    plan = build_stage_placement_plan(config)
...
def test_gpu_capacity_check_rejects_combined_overcommit(monkeypatch) -> None:
    ...
    config = PipelineConfig(
        model_path="dummy",
        stages=[
            make_stage("preprocess", gpu=0, fraction=0.90, next_stage="thinker"),
            make_stage(
                "thinker",
                gpu=0,
                kv_cache_bytes=2 * 1024**3,
...
def test_gpu_capacity_check_passes_within_card(monkeypatch) -> None:
    ...
    config = PipelineConfig(
        model_path="dummy",
        stages=[
            make_stage("preprocess", gpu=0, fraction=0.50, next_stage="thinker"),
            make_stage(
                "thinker",
                gpu=0,
                kv_cache_bytes=2 * 1024**3,
...
def test_same_gpu_placement_records_missing_memory_fraction_stages() -> None:
    config = PipelineConfig(
        ...
        stages=[
            make_stage("preprocess", gpu=0, fraction=0.10, next_stage="thinker"),
            make_stage("thinker", gpu=0, terminal=True),
        ],
    )
...
def test_same_gpu_without_budget_records_placement() -> None:
    config = PipelineConfig(
        model_path="dummy",
        stages=[
            make_stage("image_encoder", gpu=0, next_stage="thinker"),
            make_stage("thinker", gpu=0, terminal=True),
        ],
    )
...
def test_same_gpu_colocation_sums_budget() -> None:
    config = PipelineConfig(
        ...
        stages=[
            make_stage("preprocess", gpu=0, fraction=0.10, next_stage="thinker"),
            make_stage("thinker", gpu=0, fraction=0.70, terminal=True),
        ],
    )
...
def test_same_gpu_colocation_rejects_over_budget() -> None:
    config = PipelineConfig(
        model_path="dummy",
        stages=[
            make_stage("preprocess", gpu=0, fraction=0.35, next_stage="thinker"),
            make_stage("thinker", gpu=0, fraction=0.75, terminal=True),
        ],
    )
...
def test_placement_policy_hook_runs_after_generic_plan() -> None:
    config = PipelineConfig(
        ...
        stages=[make_stage("thinker", gpu=0, terminal=True)],
    )
...
def test_gpu_capacity_check_rejects_kv_pools_over_vram(monkeypatch) -> None:
    ...
    config = PipelineConfig(
        model_path="dummy",
        stages=[
            make_stage("thinker", gpu=0, kv_cache_bytes=100 * 1024**3, terminal=True),
        ],
    )
    plan = build_stage_placement_plan(config)
    monkeypatch.setattr(
...
def test_gpu_capacity_check_sums_replica_kv_pools(monkeypatch) -> None:
    ...
    config = PipelineConfig(
        model_path="dummy",
        stages=[
            make_stage("thinker", gpu=0, kv_cache_bytes=60 * 1024**3, terminal=True),
        ],
        processes={
