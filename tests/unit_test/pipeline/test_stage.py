...
def test_ipc_payload_helpers_keep_cpu_tensors_in_payload_skeleton() -> None:
    payload = make_stage_payload(
        request_id="req-ipc",
        data={
            "cpu": torch.arange(3),
    ...
    metadata = relay_io.serialize_ipc_payload(payload)
    restored = relay_io.deserialize_ipc_payload(metadata)

    assert metadata["_ipc_payload"] is True
    assert metadata["cuda_tensors"] == {}
    assert tensor_equal(restored.data, payload.data)


def test_ipc_payload_helpers_round_trip_extracted_tensor_placeholders(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    payload = make_stage_payload(
        request_id="req-ipc",
        data={
            "cuda": "placeholder-source",
            "nested": {"cpu": torch.arange(2), "cuda": "nested-source"},
        },
    )
    tensor = torch.arange(3)
    nested_tensor = torch.ones(2)
    ...
    monkeypatch.setattr(
        ...
        lambda data: (
            {
                "cuda": {"_tensor_placeholder": "cuda"},
                "nested": {
                    "cpu": data["nested"]["cpu"],
                    "cuda": {"_tensor_placeholder": "nested.cuda"},
                },
            },
            {"cuda": tensor, "nested.cuda": nested_tensor},
        ),
    )
    monkeypatch.setattr(relay_io, "ipc_pickle", relay_io.pickle.dumps)

    metadata = relay_io.serialize_ipc_payload(payload)
    restored = relay_io.deserialize_ipc_payload(metadata)

    assert set(metadata["cuda_tensors"]) == {"cuda", "nested.cuda"}
    assert restored.data["cuda"] is not tensor
    assert tensor_equal(restored.data["cuda"], tensor)
    assert tensor_equal(restored.data["nested"]["cuda"], nested_tensor)
    assert tensor_equal(restored.data["nested"]["cpu"], payload.data["nested"]["cpu"])
...
def test_stage_sends_same_gpu_full_payload_as_cuda_ipc(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    async def _run() -> None:
        relay = FakeRelay()
        ...
        payload = make_stage_payload(request_id="req-ipc", data={"hidden": "tensor"})

        monkeypatch.setattr(relay_io, "contains_ipc_payload_tensor", lambda data: True)
        monkeypatch.setattr(
            relay_io,
            "serialize_ipc_payload",
            lambda payload: {"_ipc_payload": True, "data": payload.data},
        )
        ...
        assert msg.shm_metadata == {"_ipc_payload": True, "data": {"hidden": "tensor"}}

    asyncio.run(_run())


def test_stage_same_gpu_payload_without_cuda_tensor_falls_back_to_relay(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    async def _run() -> None:
        relay = FakeRelay()
        ...
        payload = make_stage_payload(request_id="req-relay", data={"answer": 7})

        monkeypatch.setattr(relay_io, "contains_ipc_payload_tensor", lambda data: False)
        ...
        assert relay.storage
        assert len(control_plane.sent_to_stage) == 1
        assert "_ipc_payload" not in control_plane.sent_to_stage[0][2].shm_metadata

    asyncio.run(_run())


def test_stage_local_object_takes_precedence_over_same_gpu_payload_ipc(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    async def _run() -> None:
        ...
        receiver = make_stage(name="thinker", scheduler=receiver_scheduler)
        sender = make_stage(
            name="encoder",
            endpoints={"thinker": "inproc://thinker"},
            relay=relay,
        ...
        payload = make_stage_payload(request_id="req-local", data={"hidden": "tensor"})
...
def test_stage_fan_out_same_gpu_payloads_fall_back_to_relay(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    async def _run() -> None:
        relay = FakeRelay()
        ...
        sender = make_stage(
            name="encoder",
            get_next=lambda request_id, output: ["thinker", "archive"],
            endpoints={
            ...
            relay=relay,
            control_plane=control_plane,
            same_gpu_payload_targets={"thinker", "archive"},
        )
        ...
        await sender._route_result(
            "req-fanout",
            make_stage_payload(request_id="req-fanout", data={"hidden": "tensor"}),
        )
        ...
        assert all(
            "_ipc_payload" not in msg.shm_metadata
            for _, _, msg in control_plane.sent_to_stage
        )
        assert relay.storage
...
def test_stage_receives_ipc_full_payload_without_relay_read(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    async def _run() -> None:
        relay = FakeRelay()
        scheduler = FakeScheduler()
        receiver = make_stage(name="thinker", relay=relay, scheduler=scheduler)
        payload = make_stage_payload(request_id="req-ipc", data={"hidden": "tensor"})
        ...
        await receiver._on_data_ready(
            DataReadyMessage(
                ...
                from_stage="encoder",
                to_stage="thinker",
                shm_metadata={"_ipc_payload": True},
            )
        )

        assert relay.storage == {}
        assert relay.cleaned == []
