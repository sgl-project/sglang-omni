...
def make_plan(config: PipelineConfig, specs, tmp_path):
...
def test_off_plans_nothing(tmp_path) -> None:
    ...
    specs = [
        spec("gen@r0", stage_name="engine@r0", gpu_id=0),
        spec("gen@r1", stage_name="engine@r1", gpu_id=0),
    ]

    assert make_plan(config, specs, tmp_path) is None
...
def test_lowest_replica_leads_and_the_rest_follow(tmp_path) -> None:
    ...
    specs = [
        spec(f"gen@r{index}", stage_name=f"engine@r{index}", gpu_id=0)
        for index in range(3)
    ]

    plan = make_plan(config, specs, tmp_path)
...
def test_a_replica_alone_on_its_gpu_keeps_loading_normally(tmp_path) -> None:
    ...
    specs = [
        spec("gen@r0", stage_name="engine@r0", gpu_id=0),
        spec("gen@r1", stage_name="engine@r1", gpu_id=0),
        spec("gen@r2", stage_name="engine@r2", gpu_id=1),
    ]
...
def test_two_gpus_each_get_their_own_leader(tmp_path) -> None:
    ...
    specs = [
        ...
        spec("gen@r1", stage_name="engine@r1", gpu_id=0),
        spec("gen@r2", stage_name="engine@r2", gpu_id=1),
        spec("gen@r3", stage_name="engine@r3", gpu_id=1),
    ]
...
def test_every_process_gets_the_reduction_compat_flag(tmp_path) -> None:
    ...
    specs = [
        spec("gen@r0", stage_name="engine@r0", gpu_id=0),
        spec("gen@r1", stage_name="engine@r1", gpu_id=0),
        spec("vocoder", stage_name="vocoder", gpu_id=0),
    ]
...
def test_a_replicated_tp_process_is_skipped_not_rejected(tmp_path) -> None:
    ...
    specs = [
        spec("gen@r0", stage_name="engine@r0", gpu_id=0),
        spec("gen@r1", stage_name="engine@r1", gpu_id=0),
        spec("thinker@r0_tp0", stage_name="thinker@r0", gpu_id=1, tp_size=2),
        spec("thinker@r0_tp1", stage_name="thinker@r0", gpu_id=2, tp_size=2),
        spec("thinker@r1_tp0", stage_name="thinker@r1", gpu_id=1, tp_size=2),
        spec("thinker@r1_tp1", stage_name="thinker@r1", gpu_id=2, tp_size=2),
    ]
...
def test_cpu_only_replicas_are_not_a_sharing_group(tmp_path) -> None:
    ...
    specs = [
        spec("gen@r0", stage_name="engine@r0", gpu_id=0),
        spec("gen@r1", stage_name="engine@r1", gpu_id=0),
        spec("pre@r0", stage_name="preprocess@r0", gpu_id=None),
        spec("pre@r1", stage_name="preprocess@r1", gpu_id=None),
    ]
...
def test_two_engine_stages_in_one_sharing_process_are_rejected(tmp_path) -> None:
    config = make_config(
        ...
        stages=[
            engine_stage(),
            engine_stage(name="second_engine", process="gen"),
        ],
        processes={"gen": ProcessConfig(num_replicas=2, replica_devices=[0, 0])},
    )
    specs = [
