...
class ScriptedAudioDecoder:
        ...
        *,
        ...
        with self.condition:
            self.stream_calls.append(
                StreamCall(slot_ids, values, terminal_flags, thread_id)
            )
        ...
        if self.block_stream_call == call_index:
            ...
            if not self.block_release.wait(WAIT_TIMEOUT_S):
    ...
    def decode_full(self, latents: torch.Tensor) -> torch.Tensor:
        ...
        if self.block_full_call == call_index:
            ...
            if not self.full_block_release.wait(WAIT_TIMEOUT_S):
...
def drain(scheduler: MingTTSStreamingVocoderScheduler):
    messages = []
    while True:
        try:
...
def take(scheduler: MingTTSStreamingVocoderScheduler, count: int):
    return [scheduler.outbox.get(timeout=WAIT_TIMEOUT_S) for _ in range(count)]


def start(scheduler: MingTTSStreamingVocoderScheduler) -> threading.Thread:
...
def start_external_abort_after_tombstone(
    scheduler: MingTTSStreamingVocoderScheduler,
    request_id: str,
    ...
    thread.start()
    assert recorded.wait(WAIT_TIMEOUT_S)
    return thread


def test_stream_capacity_bounds_chunk_collection_not_full_batching() -> None:
    decoder = ScriptedAudioDecoder(capacity=2)
    scheduler = make_scheduler(decoder)
    chunk_messages = [
        IncomingMessage(
            f"stream-{index}",
            "stream_chunk",
            stream_item(0, index, is_last=False),
        )
    ...
    full_messages = [
        IncomingMessage(
            f"full-{index}",
            "new_request",
            payload(f"full-{index}", stream=False),
        )
...
def test_ming_stream_ingress_rejects_owned_contract_errors(
    ...
    decoder = ScriptedAudioDecoder(capacity=1)
    scheduler = make_scheduler(decoder)
    item = stream_item(0, 1.0, is_last=False)
...
def test_real_inbox_preserves_mk_terminal_backlog_exact_once() -> None:
    ...
    scheduler = make_scheduler(decoder, initial=2, steady=4)
    ...
    for chunk_id in range(2):
        scheduler.inbox.put(
            IncomingMessage(
                request_id,
                "stream_chunk",
                stream_item(chunk_id, chunk_id, is_last=False),
            )
        )
    scheduler.inbox.put(
        IncomingMessage(
    ...
    for chunk_id in range(2, 9):
        scheduler.inbox.put(
            IncomingMessage(
                request_id,
                "stream_chunk",
                stream_item(chunk_id, chunk_id, is_last=chunk_id == 8),
            )
        )
    ...
    scheduler.inbox.put(
        IncomingMessage(
            request_id,
            "new_request",
            payload(request_id, stream=True),
        )
    )

    thread = start(scheduler)
    messages = take(scheduler, 4)
...
def test_initial_cadence_can_exceed_steady_cadence() -> None:
    decoder = ScriptedAudioDecoder(capacity=1)
    scheduler = make_scheduler(decoder, initial=3, steady=1)
    scheduler.on_stream_chunk_batch(
        [
            (
                "initial-larger",
                stream_item(index, index, is_last=index == 4),
            )
...
def test_mk_mixed_wave_preserves_request_slot_and_output_mapping() -> None:
    decoder = ScriptedAudioDecoder(capacity=3)
    scheduler = make_scheduler(decoder, initial=2, steady=4)
    scheduler.on_stream_chunk_batch(
        [
            ("step", stream_item(0, 10, is_last=False)),
            ("step", stream_item(1, 11, is_last=False)),
        ]
    )
    step_slot = scheduler.slot_bindings.slot_for("step")
    first_messages = drain(scheduler)

    scheduler.on_stream_chunk_batch(
        [
            ("step", stream_item(2, 12, is_last=False)),
            ("step", stream_item(3, 13, is_last=False)),
            ("step", stream_item(4, 14, is_last=False)),
            ("step", stream_item(5, 15, is_last=False)),
            ("open-terminal", stream_item(0, 20, is_last=False)),
            ("open-terminal", stream_item(1, 21, is_last=True)),
            ("short-terminal", stream_item(0, 30, is_last=True)),
            ("waiting", stream_item(0, 40, is_last=False)),
        ]
    )
    mixed_messages = drain(scheduler)
    ...
    scheduler.on_stream_chunk_batch([("waiting", stream_item(1, 41, is_last=True))])
    waiting_messages = drain(scheduler)
...
def test_terminal_patch_bypasses_opening_threshold() -> None:
    decoder = ScriptedAudioDecoder(capacity=1)
    scheduler = make_scheduler(decoder, initial=3, steady=2)

    scheduler.on_stream_chunk_batch(
        [("short-terminal", stream_item(0, 1, is_last=True))]
    )
...
def test_late_chunk_after_terminal_is_request_local_error() -> None:
    decoder = ScriptedAudioDecoder(capacity=1)
    scheduler = make_scheduler(decoder, initial=1, steady=1)
    scheduler.on_stream_chunk_batch(
        [("late-terminal", stream_item(0, 1, is_last=True))]
    )
    drain(scheduler)

    scheduler.on_stream_chunk_batch(
        [("late-terminal", stream_item(1, 2, is_last=False))]
    )
...
def test_one_pass_admission_keeps_holder_and_uses_registry_order() -> None:
    ...
    scheduler.on_stream_chunk_batch(
        [
            ("holder", stream_item(0, 10, is_last=False)),
            ("releasing", stream_item(0, 20, is_last=False)),
        ]
    )
    ...
    scheduler.on_stream_chunk_batch(
        [
            ("waiter-first", stream_item(0, 30, is_last=True)),
            ("waiter-second", stream_item(0, 40, is_last=True)),
        ]
    )
    assert len(decoder.stream_calls) == 1

    scheduler.on_stream_chunk_batch([("releasing", stream_item(1, 21, is_last=True))])
...
def test_successful_mixed_wave_commits_terminal_before_releasing_slot(
    ...
    scheduler.on_stream_chunk_batch(
        [
            ("live", stream_item(0, 10, is_last=False)),
            ("terminal", stream_item(0, 20, is_last=False)),
        ]
    )
    ...
    scheduler.on_stream_chunk_batch([("waiter", stream_item(0, 30, is_last=True))])
    assert len(decoder.stream_calls) == 1

    scheduler.on_stream_chunk_batch(
        [
            ("live", stream_item(1, 11, is_last=False)),
            ("terminal", stream_item(1, 21, is_last=True)),
        ]
    )
...
def test_step_error_resets_participant_before_next_wave() -> None:
    decoder = ScriptedAudioDecoder(capacity=2)
    scheduler = make_scheduler(decoder, initial=1, steady=1)
    scheduler.on_stream_chunk_batch([("holder", stream_item(0, 10, is_last=False))])
    holder_slot = scheduler.slot_bindings.slot_for("holder")
    drain(scheduler)
    decoder.stream_actions.append(ValueError("invalid staged input"))

    scheduler.on_stream_chunk_batch([("offender", stream_item(0, 20, is_last=False))])
    scheduler.on_stream_chunk_batch([("waiter", stream_item(0, 30, is_last=True))])
    assert len(decoder.stream_calls) == 3
    errors = [message for message in drain(scheduler) if message.type == "error"]
...
def test_wave_failure_aborts_only_participants_and_future_wave_succeeds() -> None:
    ...
    scheduler = make_scheduler(decoder, initial=1, steady=1)
    scheduler.on_stream_chunk_batch(
        [("inactive-holder", stream_item(0, 10, is_last=False))]
    )
    ...
    scheduler.on_stream_chunk_batch(
        [
            ("failed-a", stream_item(0, 1, is_last=False)),
            ("failed-b", stream_item(0, 2, is_last=True)),
        ]
    )
    ...
    scheduler.on_stream_chunk_batch(
        [
            ("failed-a", stream_item(1, 3, is_last=True)),
            ("failed-b", stream_item(1, 4, is_last=True)),
        ]
    )
    assert len(decoder.stream_calls) == failed_call_count
    assert drain(scheduler) == []

    scheduler.on_stream_chunk_batch([("future-row", stream_item(0, 3, is_last=True))])
...
def test_terminal_without_audio_is_request_local_error() -> None:
    ...
    scheduler = make_scheduler(decoder, initial=2, steady=4)
    scheduler.on_stream_chunk_batch(
        [("silent-terminal", stream_item(0, 1, is_last=True))]
    )
    thread = start(scheduler)
    scheduler.inbox.put(
        IncomingMessage(
...
def test_slotless_terminal_full_stacks_stream_patches_exactly_once() -> None:
    decoder = ScriptedAudioDecoder(capacity=1)
    scheduler = make_scheduler(decoder, initial=1, steady=2)
    thread = start(scheduler)
    scheduler.inbox.put(
        IncomingMessage("holder", "stream_chunk", stream_item(0, 9, is_last=False))
    )
    assert take(scheduler, 1)[0].request_id == "holder"
    ...
    for chunk_id in range(3):
        scheduler.inbox.put(
            IncomingMessage(
                "slotless-full",
                "stream_chunk",
                stream_item(chunk_id, chunk_id, is_last=chunk_id == 2),
            )
        )
    ...
    scheduler.inbox.put(
        IncomingMessage(
            "slotless-full",
            "new_request",
            payload("slotless-full", stream=True),
        )
    )

    decoder.wait_for_full_calls(1)
    messages = take(scheduler, 2)
...
def test_slotless_full_failure_is_request_local(
    ...
    for chunk_id in range(2):
        scheduler.inbox.put(
            IncomingMessage(
                "failed-full",
                "stream_chunk",
                stream_item(chunk_id, chunk_id, is_last=chunk_id == 1),
            )
        )
    ...
    scheduler.inbox.put(
        IncomingMessage(
            "failed-full",
            "new_request",
            payload("failed-full", stream=True),
        )
    )

    decoder.wait_for_full_calls(1)
    error = take(scheduler, 1)
    ...
    scheduler.inbox.put(
        IncomingMessage("failed-full", "stream_chunk", stream_item(2, 10, is_last=True))
    )
    scheduler.inbox.put(
        IncomingMessage("holder", "stream_chunk", stream_item(1, 11, is_last=True))
    )
    scheduler.inbox.put(
        IncomingMessage("future", "stream_chunk", stream_item(0, 12, is_last=True))
    )
    decoder.wait_for_stream_calls(3)
    future_messages = take(scheduler, 2)
...
def test_external_abort_is_lazy_and_next_streaming_turn_resets_before_reuse() -> None:
    ...
    thread = start(scheduler)
    scheduler.inbox.put(
        IncomingMessage("holder", "stream_chunk", stream_item(0, 1, is_last=False))
    )
    assert take(scheduler, 1)[0].request_id == "holder"
    scheduler.inbox.put(
        IncomingMessage("waiter", "stream_chunk", stream_item(0, 2, is_last=True))
    )
    scheduler.inbox.put(
        IncomingMessage(
            "barrier-full",
    ...
    scheduler.inbox.put(
        IncomingMessage("waiter", "new_request", payload("waiter", stream=True))
    )
    decoder.wait_for_full_calls(2)
    messages = take(scheduler, 3)
    ...
    scheduler.inbox.put(
        IncomingMessage("future", "stream_chunk", stream_item(0, 3, is_last=True))
    )
    decoder.wait_for_stream_calls(2)
    future_message = take(scheduler, 1)
...
def test_external_abort_during_fixed_wave_suppresses_output_and_resets_after_fence(
    ...
    scheduler = make_scheduler(decoder, initial=1, steady=1)
    scheduler_thread = start(scheduler)
    scheduler.inbox.put(
        IncomingMessage(
            "aborted-in-flight",
            "stream_chunk",
            stream_item(0, 1, is_last=False),
        )
    )
    assert decoder.block_started.wait(WAIT_TIMEOUT_S)

    abort_thread = start_external_abort_after_tombstone(
    ...
    abort_thread.join(timeout=WAIT_TIMEOUT_S)
    assert not abort_thread.is_alive()
    assert drain(scheduler) == []
    assert decoder.reset_rows_calls == []

    scheduler.inbox.put(
        IncomingMessage("future", "stream_chunk", stream_item(0, 2, is_last=True))
    )
    decoder.wait_for_stream_calls(2)
    future_message = take(scheduler, 1)
    ...
    stop(scheduler, scheduler_thread)
...
def test_external_abort_during_slotless_full_suppresses_output(
    ...
    assert take(scheduler, 1)[0].request_id == "holder"
    scheduler.inbox.put(
        IncomingMessage("aborted-full", "stream_chunk", stream_item(0, 1, is_last=True))
    )
    ...
    scheduler.inbox.put(
        IncomingMessage(
            "aborted-full",
            "new_request",
            payload("aborted-full", stream=True),
        )
    )
    assert decoder.full_block_started.wait(WAIT_TIMEOUT_S)
...
def test_reset_failure_unbinds_request_and_keeps_slot_unavailable() -> None:
    ...
    scheduler.on_stream_chunk_batch([("dirty-row", stream_item(0, 1, is_last=False))])

    dirty_error = drain(scheduler)
    ...
    decoder.wait_for_full_calls(1)
    waiter_messages = take(scheduler, 2)

    scheduler.on_stream_chunk_batch([("future", stream_item(0, 3, is_last=True))])
...
def test_full_failure_does_not_affect_live_or_future_streaming() -> None:
    ...
    scheduler.inbox.put(
        IncomingMessage(
            "future-stream",
            "stream_chunk",
            stream_item(0, 3, is_last=False),
        )
    )
    future_results = take(scheduler, 2)
...
def test_done_without_terminal_is_request_local_error() -> None:
    ...
    scheduler.inbox.put(
        IncomingMessage(
            "missing-terminal",
            "new_request",
            payload("missing-terminal", stream=True),
        )
    )
...
def test_healthy_stop_resets_full_bank_once_with_live_bound_state() -> None:
    decoder = ScriptedAudioDecoder(capacity=1)
    scheduler = make_scheduler(decoder, initial=1, steady=2)
    scheduler.on_stream_chunk_batch([("live-holder", stream_item(0, 1, is_last=False))])
...
def test_stop_during_pump_drains_current_turn_only() -> None:
    decoder = ScriptedAudioDecoder(capacity=4)
    decoder.block_stream_call = 1
    scheduler = make_scheduler(decoder, initial=1, steady=2)
    ...
    for chunk_id in range(4):
        scheduler.inbox.put(
            IncomingMessage(
                request_id,
                "stream_chunk",
                stream_item(chunk_id, chunk_id, is_last=chunk_id == 3),
            )
        )
    scheduler.inbox.put(
        IncomingMessage(
