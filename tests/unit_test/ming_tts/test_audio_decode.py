...
def _accelerator_device() -> torch.device | None:
    ...
    if missing_streaming_device_api(device) is not None:
        return None
    return device


def graph_captured_fft_supported() -> bool:
    from sglang_omni import platforms

    return platforms.current_platform.supports_graph_captured_fft()
...
requires_graph_captured_fft = pytest.mark.skipif(
    not graph_captured_fft_supported(),
    reason="this platform's graphs cannot record the AudioVAE inverse FFT",
)
...
def test_the_streaming_graph_captures_and_replays_on_this_accelerator() -> None:
    ...
    runner = MingAudioStreamingRunner(transition, cuda_graph_required=True)
...
def cpu_streaming_transition() -> AudioVAEFixedStreamingTransition:
    with torch.random.fork_rng(devices=[]):
        torch.manual_seed(6)
        audio_vae = _make_tiny_audio_vae()
    transition = AudioVAEFixedStreamingTransition(
