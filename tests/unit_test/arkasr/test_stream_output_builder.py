...
def test_per_request_state_is_isolated() -> None:
    ...
    assert (
        rd1.req._arkasr_stream_pending_ids == []
    )  # noqa: leading-underscore  # upstream name
    assert (
        rd2.req._arkasr_stream_pending_ids == []
    )  # noqa: leading-underscore  # upstream name
...
def test_suppressed_marker_tokens_do_not_appear_in_deltas() -> None:
    class MarkerTokenizer(ByteTokenizer):
        all_special_ids = [EOS]
        eos_token_id = EOS
    ...
    builder = make_arkasr_stream_output_builder(
        tokenizer=MarkerTokenizer({1: b"hello", 2: b"<tool_call>", 3: b"<|audio|>"}),
    )
    rd = make_req_data()

    assert [m.data["text"] for m in builder("r", rd, make_req_output(1))] == ["hello"]
    assert builder("r", rd, make_req_output(2)) == []
...
def test_concatenated_deltas_strip_matches_result_adapter_text() -> None:
    ...
    tokenizer = Tok({1: b" hello", 2: b" world ", 3: b"<tool_call>", EOS: b"<eos>"})
    ...
    deltas: list[str] = []
    for token_id in (1, 3, 2, EOS):
        deltas.extend(
            m.data["text"] for m in builder("r", rd, make_req_output(token_id))
        )
