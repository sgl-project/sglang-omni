...
def _request_data(result) -> object:
...
def test_arkasr_embedding_cache_hit_skips_mel_extraction(
    ...
    class _EncoderService:
        def __init__(self) -> None:
            ...
            self.submitted_cached_embedding: torch.Tensor | None = None
        ...
        def submit_item(
            ...
            *,
            ...
            self.submitted_cached_embedding = cached_embedding
            assert cached_embedding is self.embedding
            self.attach_embedding(item, cached_embedding)
            return _completed_future(cached_embedding)
    ...
    data = _request_data(request_builder(_payload(f"cache-hit-{num_samples}")))
    ...
    assert encoder_service.submitted_cached_embedding is encoder_service.embedding
...
def test_arkasr_embedding_cache_miss_extracts_and_encodes(
    ...
    class _EncoderService:
        ...
        def submit_item(
            ...
            *,
            ...
            item.precomputed_embeddings = embedding
            item.feature = None
            return _completed_future(embedding)
    ...
    data = _request_data(request_builder(_payload("cache-miss")))

    item = data.req.multimodal_inputs.mm_items[0]
    assert encoder_service.lookup == (data.req.extra_key, 12)
