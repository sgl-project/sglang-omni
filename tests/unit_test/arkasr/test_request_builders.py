...
"""Tests for ARK-ASR request building and embedding-cache admission."""
...
def test_embedding_cache_hit_skips_mel_extraction(monkeypatch) -> None:
    ...
    service = HitEncoderService(expected_tokens=12)
    ...
    result = request_builder(payload("early-hit"))
...
def test_embedding_cache_hit_uses_truncated_frame_count(monkeypatch) -> None:
    ...
    service = HitEncoderService(expected_tokens=375)
    request_builder, _ = make_arkasr_scheduler_adapters(
        tokenizer=FakeTokenizer(),
        max_new_tokens=32,
        feature_extractor=UnexpectedFeatureExtractor(),
    ...
    result = request_builder(payload("truncated-hit"))
...
def test_estimated_tokens_match_real_whisper_boundaries(monkeypatch) -> None:
    ...
    class BoundaryService(MissEncoderService):
        def __init__(self) -> None:
            super().__init__()
    ...
    for samples in (2399, 2400, 479999, 480000, 480001, 481120):
        ...
        result = request_builder(payload(f"boundary-{samples}"))
...
def test_embedding_cache_miss_preserves_mel_and_deferred_admission(
    ...
    result = request_builder(payload("early-miss"))
...
def test_request_without_encoder_service_inherits_scheduler_policy(monkeypatch) -> None:
    ...
    data = request_builder(payload("no-pre-lm-service"))
