...
TWO_CPUS = (
    HAS_PROC and hasattr(os, "sched_getaffinity") and len(os.sched_getaffinity(0)) >= 2
)


def test_parse_cpu_list_ranges_and_singles():
    assert parse_cpu_list("48-51,112") == {48, 49, 50, 51, 112}


def test_tree_pids_follows_descendants_only():
    procs = {
        1: Proc(ppid=0, ticks=0),
        10: Proc(ppid=1, ticks=0),
        20: Proc(ppid=10, ticks=0),
        30: Proc(ppid=20, ticks=0),
        99: Proc(ppid=1, ticks=0),
    }
    assert tree_pids(procs, 10) == {10, 20, 30}


def test_foreign_ticks_deducts_tree_and_clamps():
    prev = {10: Proc(ppid=1, ticks=100, start=5)}
    ...
    cur = {10: Proc(ppid=1, ticks=300, start=5), 20: Proc(ppid=1, ticks=50)}
...
def test_foreign_ticks_deducts_reaped_child_via_parent_cutime():
    ...
    prev = {10: Proc(ppid=1, ticks=100, child_ticks=50, start=5)}
    cur = {10: Proc(ppid=1, ticks=110, child_ticks=450, start=5)}
...
def test_foreign_ticks_pid_reuse_is_a_new_process():
    ...
    prev = {10: Proc(ppid=1, ticks=100000, start=5)}
    cur = {10: Proc(ppid=1, ticks=30, start=999)}
    assert foreign_ticks(100, prev, cur, tree={10}) == 70


@pytest.mark.skipif(not HAS_PROC, reason="requires Linux procfs")
...
def sampler_with(samples: list[float]) -> ContentionSampler:
...
def test_summary_statistics_over_recorded_windows():
    sampler = sampler_with([0.0, 1.0, 2.0])
...
def test_sustained_foreign_cores_reads_the_tail(samples, sustained):
    assert sampler_with(samples).sustained_foreign_cores(3, 2.0) is sustained


def spin(core: int) -> subprocess.Popen:
...
def idlest_cores(candidates: list[int], count: int) -> list[int]:
...
@pytest.mark.skipif(not TWO_CPUS, reason="requires Linux procfs and two allowed CPUs")
def test_controlled_load_inside_cpuset_counts_outside_does_not():
    ...
    # on a shared machine.
    core_in, core_out = idlest_cores(sorted(os.sched_getaffinity(0)), 2)
    session_root = subprocess.Popen(
        [sys.executable, "-c", "import time; time.sleep(30)"]
    )
    inside = spin(core_in)
    outside = spin(core_out)
