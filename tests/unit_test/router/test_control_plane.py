...
def cp_app(tmp_path: Path, upstream: MockUpstream | None = None, **kwargs):
    ...
    app = create_control_plane_app(
        make_config(**{k: v for k, v in kwargs.items() if k == "failure_threshold"}),
...
def test_cp_publishes_snapshot_on_startup_and_crud(tmp_path: Path) -> None:
    app, snapshot_path, _ = cp_app(tmp_path)
    with TestClient(app) as client:
        first = read_snapshot(snapshot_path)
        ...
        after_add = read_snapshot(snapshot_path)
        ...
        after_disable = read_snapshot(snapshot_path)
        ...
        after_delete = read_snapshot(snapshot_path)
...
def test_cp_restart_recovers_worker_crud(tmp_path: Path) -> None:
    app, snapshot_path, upstream = cp_app(tmp_path)
    with TestClient(app) as client:
        assert (
            client.post(
                "/workers",
    ...
    with TestClient(restarted) as client:
        ...
        assert read_snapshot(snapshot_path).cp_epoch == "epoch-restarted"
...
def test_cp_journal_lands_in_the_configured_state_dir_not_the_workdir(
    ...
    workdir.mkdir()
    config = make_config().model_copy(update={"router_state_dir": str(state_dir)})

    app = create_control_plane_app(
        config,
        snapshot_path=str(workdir / "workers.json"),
        cp_epoch="epoch-1",
        client=httpx.AsyncClient(transport=httpx.MockTransport(MockUpstream().handler)),
    )
...
def test_worker_failure_report_evicts_at_threshold_and_republishes(
    ...
    app, snapshot_path, _ = cp_app(tmp_path, failure_threshold=1)
    with TestClient(app) as client:
        snapshot = read_snapshot(snapshot_path)
        ...
        response = client.post(
            ...
            json=make_failure(worker_id, seq=1),
        )
        ...
        assert (
            client.post(
                "/internal/worker_failure", json=make_failure("missing", seq=2)
            ).status_code
            == 404
        )
...
def test_repeat_failures_on_an_unhealthy_worker_do_not_republish(
    ...
    with TestClient(app) as client:
        ...
        assert (
            client.post(
                "/internal/worker_failure", json=make_failure(worker_id, seq=1)
            ).status_code
            == 200
        )
        seq_after_eviction = read_snapshot(snapshot_path).seq

        assert (
            client.post(
                "/internal/worker_failure", json=make_failure(worker_id, seq=2)
            ).status_code
            == 200
        )
        assert read_snapshot(snapshot_path).seq == seq_after_eviction


def test_internal_routes_on_cp_require_the_token_when_configured(
    tmp_path: Path,
) -> None:
    app, snapshot_path, _ = cp_app(tmp_path, internal_token="cp-secret")
    with TestClient(app) as client:
        snapshot = read_snapshot(snapshot_path)
        worker_id = snapshot.workers[0].worker_id
        body = make_failure(worker_id, seq=1)
...
def await_disabled_snapshot_seq(app, timeout: float = 10.0) -> int:
...
def test_weight_update_waits_for_dp_ack_then_broadcasts(tmp_path: Path) -> None:
    ...
    app, snapshot_path, _ = cp_app(tmp_path, upstream=upstream)
    with TestClient(app) as client:
        client.post(
        ...
        update.start()
        try:
            seq = await_disabled_snapshot_seq(app)
...
def test_weight_update_fails_closed_when_a_live_dp_never_acks(
    ...
    upstream = MockUpstream()
    app, snapshot_path, _ = cp_app(tmp_path, upstream=upstream, dp_ack_timeout_secs=0.2)
    with TestClient(app) as client:
        client.post(
            "/internal/register", json={"dp_index": 0, "generation": 1, "pid": 1}
        )
        ...
        assert read_snapshot(snapshot_path).workers[0].disabled is False
...
def test_cp_admin_surface_enforces_the_admin_key(tmp_path: Path) -> None:
    app, _, _ = cp_app(tmp_path, admin_api_key="cp-admin-key")
    with TestClient(app) as client:
...
def test_cp_aggregates_dp_counters_into_worker_listings(tmp_path: Path) -> None:
    ...
    with TestClient(app) as client:
        ...
        def report(seq: int, routed: int, ok: int, bad: int, active: int) -> dict:
            return {
                "dp_index": 0,
                "generation": 1,
                "counter_seq": seq,
        ...
        # displayed
        assert (
            client.post("/internal/counters", json=report(1, 0, 0, 0, 0)).status_code
            == 200
        )
        assert (
            client.post("/internal/counters", json=report(2, 7, 6, 1, 2)).status_code
            == 200
        )
        ...
        # gen >= 1)
        stale = {**report(9, 9, 9, 0, 0), "generation": 1}
        client.post(
            "/internal/counters", json={**report(1, 1, 1, 0, 0), "generation": 2}
        )
        assert client.post("/internal/counters", json=stale).status_code == 409


def preack_future_seq(client, dp_index: int = 0) -> None:
...
def test_a_stale_epoch_seq_cannot_acknowledge_the_barrier(tmp_path: Path) -> None:
    ...
    upstream = MockUpstream()
    app, _, _ = cp_app(tmp_path, upstream=upstream, dp_ack_timeout_secs=0.2)
    with TestClient(app) as client:
        client.post(
            "/internal/register", json={"dp_index": 0, "generation": 1, "pid": 1}
        )
...
def test_barrier_fails_closed_while_an_expected_dp_is_absent(
    ...
    with TestClient(app) as client:
        preack_future_seq(client, dp_index=0)  # dp 1 never registers
        response = client.post("/update_weights_from_disk", json={"path": "/m"})
        assert response.status_code == 503
...
def test_worker_crud_is_rejected_while_an_update_holds_the_lock(
    ...
    app, _, _ = cp_app(tmp_path)
    with TestClient(app) as client:
        lock = app.state.admin_update_lock
        asyncio.run(hold(lock))  # leaves the lock held
        try:
...
async def hold(lock: asyncio.Lock) -> None:
    await lock.acquire()


def ready_heartbeat(client, dp_index: int, pid: int) -> None:
    client.post(
        "/internal/register",
...
def test_cp_health_reports_degraded_and_unhealthy_dp_states(
    ...
    app, _, _ = cp_app(tmp_path, expected_data_planes=2)
    with TestClient(app) as client:
...
def make_failure(worker_id: str, *, seq: int, dp: int = 0, generation: int = 1) -> dict:
    return {
        "worker_id": worker_id,
        "status_code": 502,
        "dp_index": dp,
...
def test_replacement_generation_report_racing_its_register_is_not_fenced(
    ...
    with TestClient(app) as client:
        ...
        worker_id = read_snapshot(snapshot_path).workers[0].worker_id
        newer = {
            "dp_index": 0,
            "generation": 2,
            "counter_seq": 1,
...
def test_redelivered_failure_events_are_counted_once(tmp_path: Path) -> None:
    ...
    app, snapshot_path, _ = cp_app(tmp_path, failure_threshold=2)
    with TestClient(app) as client:
        worker_id = read_snapshot(snapshot_path).workers[0].worker_id
        ...
        # 2
        client.post("/internal/worker_failure", json=make_failure(worker_id, seq=2))
        workers = client.get("/workers").json()["workers"]
        assert workers[0]["health_state"] == "unhealthy"
...
def test_stale_incarnation_failures_cannot_evict_a_readded_worker(
    ...
    with TestClient(app) as client:
        worker_id = read_snapshot(snapshot_path).workers[0].worker_id
        old_incarnation = read_snapshot(snapshot_path).workers[0].incarnation
        assert old_incarnation

        client.delete(f"/workers/{worker_id}")
        client.post("/workers", json={"url": "http://worker-a:8101"})
        new_incarnation = read_snapshot(snapshot_path).workers[-1].incarnation
        assert new_incarnation != old_incarnation

        stale = {**make_failure(worker_id, seq=1), "incarnation": old_incarnation}
        response = client.post("/internal/worker_failure", json=stale)
        assert response.json().get("stale_incarnation") is True
        workers = client.get("/workers").json()["workers"]
        assert workers[0]["health_state"] != "unhealthy"

        current = {**make_failure(worker_id, seq=2), "incarnation": new_incarnation}
...
def test_old_generation_reports_are_fenced(tmp_path: Path) -> None:
    ...
    with TestClient(app) as client:
        ...
        failure = make_failure(worker_id, seq=1)
