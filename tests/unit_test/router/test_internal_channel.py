...
def hello(index: int = 0, generation: int = 1, pid: int = 100) -> dict:
    return {"dp_index": index, "generation": generation, "pid": pid}


def make_beat(
    index: int = 0, generation: int = 1, pid: int = 100, seq: int = 0
) -> dict:
    return {
        "dp_index": index,
...
def test_register_then_heartbeat_updates_last_applied_seq() -> None:
    ...
    with TestClient(app) as client:
        assert client.post("/internal/register", json=hello()).status_code == 200
        response = client.post("/internal/heartbeat", json=make_beat(seq=17))
...
def test_heartbeat_before_register_asks_for_registration() -> None:
    ...
    app = create_internal_app()
    with TestClient(app) as client:
        response = client.post("/internal/heartbeat", json=make_beat())
...
def test_stale_generation_is_fenced_out() -> None:
    ...
    with TestClient(app) as client:
        assert (
            client.post("/internal/register", json=hello(generation=2)).status_code
            == 200
        )
        assert (
            client.post("/internal/register", json=hello(generation=1)).status_code
            == 409
        )
        assert (
            client.post(
                "/internal/heartbeat", json=make_beat(generation=1, seq=99)
            ).status_code
            == 409
        )
        listed = client.get("/internal/data_planes").json()["data_planes"]
...
def test_newer_generation_register_replaces_the_record() -> None:
    ...
    with TestClient(app) as client:
        client.post("/internal/register", json=hello(generation=1, pid=100))
        client.post("/internal/register", json=hello(generation=2, pid=200))
...
def test_internal_token_is_required_when_configured() -> None:
    ...
    with TestClient(app) as client:
        assert client.post("/internal/register", json=hello()).status_code == 403
        assert (
            client.post(
                "/internal/register",
                json=hello(),
...
def test_drop_removes_a_data_plane() -> None:
    ...
    app = create_internal_app(state)
    with TestClient(app) as client:
        client.post("/internal/register", json=hello())
        state.drop(0)
...
def test_equal_generation_register_from_another_pid_is_fenced() -> None:
    ...
    with TestClient(app) as client:
        assert (
            client.post(
                "/internal/register", json=hello(generation=1, pid=100)
            ).status_code
            == 200
        )
        assert (
            client.post(
                "/internal/register", json=hello(generation=1, pid=999)
            ).status_code
            == 409
        )
...
def test_same_pid_reregister_is_idempotent_and_keeps_the_ack() -> None:
    ...
    with TestClient(app) as client:
        client.post("/internal/register", json=hello())
        beat = {**make_beat(seq=17), "last_applied_epoch": "epoch-x", "serving": False}
...
def test_heartbeat_with_a_future_generation_requires_registration() -> None:
    app = create_internal_app()
    with TestClient(app) as client:
        client.post("/internal/register", json=hello(generation=1))
        response = client.post("/internal/heartbeat", json=make_beat(generation=2))
        assert response.status_code == 428
...
def test_heartbeat_from_the_wrong_pid_is_fenced() -> None:
    app = create_internal_app()
    with TestClient(app) as client:
        client.post("/internal/register", json=hello(pid=100))
        assert (
            client.post("/internal/heartbeat", json=make_beat(pid=999)).status_code
            == 409
        )
...
def test_replayed_heartbeat_cannot_regress_the_ack() -> None:
    ...
    with TestClient(app) as client:
        client.post("/internal/register", json=hello())
        client.post("/internal/heartbeat", json=make_beat(seq=17))
        assert (
            client.post("/internal/heartbeat", json=make_beat(seq=3)).status_code == 200
        )
        listed = client.get("/internal/data_planes").json()["data_planes"]
        assert listed[0]["last_applied_seq"] == 17
...
def test_identity_fields_have_lower_bounds() -> None:
    ...
    with TestClient(app) as client:
        assert (
            client.post("/internal/register", json=hello(generation=0)).status_code
            == 422
        )
        assert client.post("/internal/register", json=hello(pid=0)).status_code == 422
