...
def test_first_contact_establishes_the_baseline() -> None:
    ...
    assert ledger.apply(make_report(1, 20), now=0.0) is True
    assert ledger.totals("w0")["routed_total"] == 0
    ledger.apply(make_report(2, 25), now=0.0)
...
def test_a_regressed_cumulative_cannot_lower_the_display() -> None:
    ...
    ledger = DataPlaneCounterLedger()
    ledger.apply(make_report(1, 0), now=0.0)
    ledger.apply(make_report(2, 10), now=0.0)
    assert ledger.apply(make_report(3, 3), now=0.0) is True  # newer seq, lower value
...
def test_workers_missing_from_a_report_keep_their_contribution() -> None:
    ledger = DataPlaneCounterLedger()
    ledger.apply(make_report(1, 0), now=0.0)
    ledger.apply(make_report(2, 7), now=0.0)
...
def test_generation_bump_retires_the_old_contribution() -> None:
    ...
    ledger = DataPlaneCounterLedger()
    ledger.apply(make_report(1, 2, generation=1), now=0.0)  # baseline 2
    ledger.apply(make_report(7, 10, generation=1), now=0.0)  # contribution 8
    ...
    ledger.apply(make_report(1, 3, generation=2), now=0.0)
    assert ledger.totals("w0")["routed_total"] == 11
    ledger.apply(make_report(2, 4, generation=2), now=0.0)
...
def test_only_the_first_report_from_a_data_plane_is_a_baseline() -> None:
    ...
    ledger = DataPlaneCounterLedger()
    ledger.apply(make_report(9, 100, generation=4), now=0.0)
    assert ledger.totals("w0")["routed_total"] == 0
    ledger.apply(make_report(10, 105, generation=4), now=0.0)
    assert ledger.totals("w0")["routed_total"] == 5


def test_older_generation_reports_are_fenced() -> None:
    ledger = DataPlaneCounterLedger()
    ledger.apply(make_report(1, 1, generation=2), now=0.0)
    with pytest.raises(StaleCounterGenerationError):
        ledger.apply(make_report(9, 9, generation=1), now=0.0)


def test_active_gauge_sums_only_live_entries() -> None:
    ledger = DataPlaneCounterLedger(liveness_secs=3.0)
    ledger.apply(make_report(1, 1, dp=0, active=2), now=100.0)
    ledger.apply(make_report(1, 1, dp=1, active=3), now=102.0)
...
def test_incarnation_change_retires_the_old_segment_and_counts_the_new() -> None:
    ...
    ledger = DataPlaneCounterLedger()
    ledger.apply(make_report(1, 0, incarnation="inc-a"), now=0.0)  # baseline 0
    ledger.apply(make_report(2, 10, incarnation="inc-a"), now=1.0)
    assert ledger.totals("w0")["routed_total"] == 10

    # Note (Jiaxin Deng): same stable id, new incarnation, cumulative restarted at 3
    ledger.apply(make_report(3, 3, incarnation="inc-b"), now=2.0)
    ...
    ledger.apply(make_report(4, 5, incarnation="inc-b"), now=3.0)
...
def test_overlay_renders_counter_fields() -> None:
    ...
    ledger = DataPlaneCounterLedger()
    ledger.apply(make_report(1, 0, active=0))
    ledger.apply(make_report(2, 6, active=1))  # real clock: entry counts as live
    overlay = ledger.overlay(Worker())
    assert overlay == {
        "active_requests": 1,
        "routed_requests": 6,
...
def two_incarnation_report(seq: int, rows: list[tuple[str, int]]) -> CounterReport:
    return CounterReport(
        dp_index=0,
        generation=1,
        counter_seq=seq,
...
def test_a_report_carrying_both_incarnations_does_not_double_count() -> None:
    ...
    ledger = DataPlaneCounterLedger()
    ledger.apply(two_incarnation_report(1, [("inc-a", 1)]), now=0.0)
    assert ledger.totals("w0")["routed_total"] == 0  # first contact is baseline

    ledger.apply(two_incarnation_report(2, [("inc-b", 1), ("inc-a", 1)]), now=1.0)
    assert ledger.totals("w0")["routed_total"] == 1

    ledger.apply(two_incarnation_report(3, [("inc-b", 2)]), now=2.0)
...
def test_a_drained_incarnation_is_folded_and_dropped() -> None:
    ...
    ledger = DataPlaneCounterLedger()
    ledger.apply(two_incarnation_report(1, [("inc-a", 0)]), now=0.0)
    ledger.apply(two_incarnation_report(2, [("inc-b", 3), ("inc-a", 2)]), now=1.0)
    ledger.apply(two_incarnation_report(3, [("inc-b", 3)]), now=2.0)

    assert ledger.totals("w0")["routed_total"] == 5
    entry = ledger._entries[0]  # noqa: leading-underscore  # production name
