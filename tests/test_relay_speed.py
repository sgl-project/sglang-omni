...
from sglang_omni.relay.nixl import NixlRelay, NIXL_AVAILABLE
        ...
        for i in range(NUM_ITERS):
            ...
            if NIXL_AVAILABLE:
                ...
                if dest_tensor[0].item() == expected_val:
                    ...
                    print(f"❌ [Iter {i}] Failed! Exp: {expected_val}, Got: {dest_tensor[0].item()}")
            else:
...
if __name__ == "__main__":
    ...
    p_sender.start()
    time.sleep(2) # Give Sender more initialization time (large memory registration is slow)
    p_receiver.start()
