...
def launch_serve(port: int, mps: str) -> subprocess.Popen:
...
def terminate(proc: subprocess.Popen, timeout: float = 120.0) -> None:
...
def process_identity(pid: int) -> tuple[ProcessIdentity, str] | None:
    try:
        fields = Path(f"/proc/{pid}/stat").read_text().rsplit(")", 1)[1].split()
        return ProcessIdentity(pid, int(fields[19])), fields[0]
    except (FileNotFoundError, IndexError, ValueError):
        return None


def mps_process_identities() -> set[ProcessIdentity]:
    ...
    for environ_file in Path("/proc").glob("[0-9]*/environ"):
        ...
        if str(STATE_ROOT).encode() not in env or b"nvidia-cuda-mps" not in cmd:
            continue
        identity_and_state = process_identity(int(environ_file.parent.name))
        if identity_and_state is not None:
            identities.add(identity_and_state[0])
    return identities


def assert_process_identities_gone(
    identities: set[ProcessIdentity],
    timeout_s: float = 10.0,
) -> None:
    deadline = time.monotonic() + timeout_s
    while True:
        leftovers: dict[ProcessIdentity, str] = {}
...
def wait_gpu_drained(timeout_s: int = 180) -> None:
...
def clean_state_root(monkeypatch):
    ...
    assert_no_residue()
    stale_daemons = list_daemon_pids()
    ...
    mps_processes: set[ProcessIdentity] = set()
    serve = launch_serve
    daemon_pids = list_daemon_pids

    def tracked_daemon_pids() -> set[int]:
        mps_processes.update(mps_process_identities())
        return daemon_pids()
    ...
    monkeypatch.setattr(sys.modules[__name__], "list_daemon_pids", tracked_daemon_pids)
    yield
    ...
    if list(STATE_ROOT.glob("*/pipe")):
        operator_cleanup(session_ids)
    else:
        signal_test_sessions(session_ids, signal.SIGKILL)
        assert_process_identities_gone(mps_processes)
        shutil.rmtree(STATE_ROOT, ignore_errors=True)
    assert_process_identities_gone(mps_processes)


def signal_test_sessions(session_ids: Iterable[int], sig: int) -> set[int]:
...
def operator_cleanup(session_ids: int | Iterable[int]) -> None:
    ...
    mps_processes = mps_process_identities()
    ...
    live_sessions = signal_test_sessions(sessions, signal.SIGSTOP)

    if not pipe_dirs or not list_daemon_pids():
        signal_test_sessions(live_sessions, signal.SIGKILL)
        assert_process_identities_gone(mps_processes)
        shutil.rmtree(STATE_ROOT, ignore_errors=True)
        return
...
def test_hard_kill_fails_fast_until_operator_cleans():
    port = find_free_port()
    proc = launch_serve(port, "auto")
    try:
        wait_healthy(port, proc)
        assert list_daemon_pids(), "no MPS daemon started for the colocated pipeline"
        request_ok(port)
        ...
        operator_cleanup(dirty_session)
        port = find_free_port()
        proc = launch_serve(port, "auto")
        wait_healthy(port, proc)
        request_ok(port)
        ...
        terminate(proc)
    assert_no_residue()
    assert not list_daemon_pids(), "daemon outlived the last serve"


def test_two_serves_share_one_daemon():
    port_a, port_b = find_free_port(), find_free_port()
    proc_a = launch_serve(port_a, "on")
    proc_b: subprocess.Popen | None = None
    try:
        wait_healthy(port_a, proc_a)
        proc_b = launch_serve(port_b, "on")
        wait_healthy(port_b, proc_b)

        daemons = list_daemon_pids()
        ...
        terminate(proc_a)
        assert list_daemon_pids() == daemons, "daemon died when one owner left"
        request_ok(port_b)
    finally:
        ...
        if proc_a.poll() is None:
            terminate(proc_a)
    assert_no_residue()
    assert not list_daemon_pids(), "daemon outlived the last owner"
