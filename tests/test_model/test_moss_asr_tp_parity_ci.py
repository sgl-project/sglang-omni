...
def _post_chat(port: int, payload: dict, timeout: float = 180.0) -> Any:
...
RUN_FLAG = os.environ.get("RUN_MOSS_TD_TP_PARITY", "0") == "1"


def _post_audio(port: int, audio_path: Path, timeout: float = 180.0) -> Any:
    payload = {
        "model": MODEL_NAME,
        "messages": [{"role": "user", "content": "<|audio_pad|> transcribe"}],
        "audios": [str(audio_path)],
    }
    return _post_chat(port, payload, timeout=timeout)
...
def start_server(
    ...
    # Write a temporary config file to pass GPU list properly
    import json, tempfile
    gpu_list = [int(d) for d in cuda_visible_devices.split(",") if d.strip()]
    config = {
    ...
    config_path = tmp_path / f"config_tp{tp_size}.yaml"
    import yaml
