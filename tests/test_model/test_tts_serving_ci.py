...
MIN_WORKER_TRAFFIC_SHARE = 0.10

SERVING_MIXED_SPEECH_NORMAL_LATENCY_P95_S_REF: float | None = None
SERVING_MIXED_SPEECH_NORMAL_RTF_P95_REF: float | None = None
SERVING_MIXED_REST_STREAM_TTFA_P95_S_REF: float | None = None
SERVING_MIXED_REST_STREAM_INTER_CHUNK_P95_S_REF: float | None = None
SERVING_MIXED_REST_STREAM_LATENCY_P95_S_REF: float | None = None
SERVING_MIXED_REST_STREAM_RTF_P95_REF: float | None = None
SERVING_MIXED_BATCH32_LATENCY_P95_S_REF: float | None = None
SERVING_MIXED_WS_NORMAL_TTFA_P95_S_REF: float | None = None
SERVING_MIXED_WS_NORMAL_LATENCY_P95_S_REF: float | None = None
SERVING_MIXED_WS_STREAM_TTFA_P95_S_REF: float | None = None
SERVING_MIXED_WS_STREAM_INTER_CHUNK_P95_S_REF: float | None = None
SERVING_MIXED_WS_STREAM_LATENCY_P95_S_REF: float | None = None
SERVING_MIXED_WS_STREAM_RTF_P95_REF: float | None = None
SERVING_MIXED_LONG_PROMPT_TOKENS_MIN_REF: float | None = None
SERVING_MIXED_LONG_COMPLETION_TOKENS_MIN_REF: float | None = None
SERVING_MIXED_LONG_LATENCY_P95_S_REF: float | None = None
SERVING_MIXED_LONG_AUDIO_DURATION_MIN_S_REF: float | None = None
SERVING_MIXED_LONG_OUTPUT_TOK_PER_REQ_S_REF: float | None = None
...
def _check_router(
    ...
    routed = [item["routed"] for item in deltas]
    total_routed = sum(routed)
    checks.check(
        total_routed > 0
        and all(count / total_routed >= MIN_WORKER_TRAFFIC_SHARE for count in routed),
        f"both workers must serve at least {MIN_WORKER_TRAFFIC_SHARE:.0%} "
        f"of traffic: {routed}",
    )
    ...
    for service_class, minimum_requests in minimum_class_requests.items():
        ...
        checks.check(
            class_total > 0
            and all(
                count / class_total >= MIN_WORKER_TRAFFIC_SHARE
                for count in class_counts
            ),
            f"both workers must serve at least {MIN_WORKER_TRAFFIC_SHARE:.0%} "
            f"of {service_class} traffic: {class_counts}",
        )
    voice_counts = {
