...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--prompt", type=str,
                        default="Hello! Tell me something interesting.")
    parser.add_argument("--system", type=str,
                        default="You are a friendly assistant. Speak naturally and warmly.")
    parser.add_argument("--max-new-tokens", type=int, default=64)
    parser.add_argument("--temperature", type=float, default=0.7)
    parser.add_argument("--relay-backend", type=str, default="shm",
                        choices=["nixl", "shm"])
...
def _save_audio(result: dict, output_path: str) -> None:
    ...
    for stage_name, payload in result.items():
        ...
        logger.info("Audio saved: %s (%.2fs, %d Hz)",
                     output_path, len(waveform_int16) / sample_rate, sample_rate)
        return
