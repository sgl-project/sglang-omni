#!/bin/bash
# No microphone, network, clipboard access or input events.
set -euo pipefail
...
xcrun swiftc -swift-version 5 -parse-as-library -target arm64-apple-macosx14.0 \
  ...
  "$project_dir/Tests/ResultWindowFeedback_test.swift" \
  -o "$test_dir/ResultWindowFeedback_test"
/usr/bin/codesign --force --sign - "$test_dir/ResultWindowFeedback_test"
"$test_dir/ResultWindowFeedback_test"
