#!/bin/bash
# Synthetic editor and isolated pasteboard only; never emits real input events.
set -euo pipefail
...
xcrun swiftc -whole-module-optimization -swift-version 5 -parse-as-library -target arm64-apple-macosx14.0 \
    ...
    "$project_dir/Tests/RevisionTarget_test.swift" -o "$test_dir/RevisionTarget_test"
/usr/bin/codesign --force --sign - "$test_dir/RevisionTarget_test"
"$test_dir/RevisionTarget_test"
