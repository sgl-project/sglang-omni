...
ASR 就绪等待时间为 `SGLANG_OMNI_STARTUP_TIMEOUT`（默认 600 秒）加 60 秒，
为导入、初始化和 HTTP 启动留出余量；模型初始化较慢时可在启动前调大该环境变量。
Ollama 的就绪等待独立限制为 300 秒。

每个新启动的服务由 Python 标准库包装器管理独立进程组。停止时先给后端最多
30 秒正常退出，再终止该组残留 worker，额外等待 5 秒后强制清理。
后端主进程异常退出时，包装器也会清理该组；复用的外部服务不在清理范围内。

服务日志位于 `examples/macos_dictation/.build/runtime-logs/`，不提交到 Git。
这是后端进程的标准输出和错误输出，内容由后端决定；客户端仍不保存听写历史。

完成听写后，可在原输入框双击同一侧 Option，录下修改意见，再双击结束。
普通语义更正需要本机 Ollama，但不要求开启“轻度整理”；少量明确拼写和字面指令可由客户端直接处理。详细操作及自动替换的
适用范围见[语音更正说明](VOICE_CORRECTION.zh-CN.md)。
