...
for (const rate of [16000, 44100, 48000]) {
  test(`${rate} Hz capture preserves duration across worklet boundaries`, () => {
    ...
    const frames = [];
    for (let start = 0; start < count; start += 128) framer.push(input.subarray(start, start + 128), f => frames.push(f));
    assert.equal(frames.length, 2);
    assert.equal(framer.offset, 0);
    for (const frame of frames) assert.ok(frame.every(v => Math.abs(v - 16384) <= 1));
  });
}

test('fractional resampling is independent of client block boundaries', () => {
  const input = Float32Array.from({length: 44100}, (_, i) => Math.sin(i * 0.05));
  function sample(block) {
    const framer = new PCMFramer(44100);
    const frames = [];
    for (let start = 0; start < input.length; start += block) framer.push(input.subarray(start,start + block), f => frames.push(...f));
    return frames;
  }
...
test('PCM transport preserves signed little-endian values', () => {
  ...
  assert.deepEqual([...decoded].map(x => Math.round(x * 32768)), [...values]);
...
test('playback schedules contiguous audio at the output sample rate', () => {
  const ctx = fakeContext(); const acks = [];
