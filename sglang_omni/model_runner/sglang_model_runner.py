...
if KVCacheConfigurator is not None:
...
class SGLModelRunner(ModelRunner):
    ...
    def __init__(
        ...
        if "ps" in model_runner_parameters:
            if ParallelState is None:
                raise RuntimeError(
                    "Installed SGLang ModelRunner requires ps but does not expose "
                    "ParallelState"
                )
    ...
    def init_cuda_graphs(self, capture_decode_cuda_graph: bool = True):
        ...
        try:
            ...
            logger.info(
                "SGLang runtime_context is unavailable; skipping capture flag "
                "re-seed for this SGLang version."
            )
        else:
            get_flags().capture.enable_torch_compile = bool(
                self.server_args.enable_torch_compile
            )
        parent_init_graphs = getattr(super(), "init_cuda_graphs", None)
        if callable(parent_init_graphs):
            result = parent_init_graphs(capture_decode_cuda_graph)
        else:
            parent_init_graphs = getattr(super(), "init_device_graphs", None)
            if callable(parent_init_graphs):
                result = parent_init_graphs()
            else:
                logger.info(
                    "SGLang ModelRunner has no graph initialization API; reusing "
                    "eager initialization completed during construction."
                )
                return None
    ...
    def init_kv_cache_configurator(self):
        ...
        """
        if KVCacheConfigurator is None:
            parent_init = getattr(super(), "init_kv_cache_configurator", None)
            if callable(parent_init):
                parent_init()
            return
