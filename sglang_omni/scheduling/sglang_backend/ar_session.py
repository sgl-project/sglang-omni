...
class ARSessionBridge:
    ...
    def accept(self, payload: StagePayload) -> None:
        ...
        if owner.unit is not None:
            if owner.unit.rid == payload.request_id:
                return
    ...
    def capacity_error(self, rid: str) -> str | None:
        ...
        unallocated = sum(
            other is not owner
            and other.unit.req is not None
            and not other.unit.req.kv.holds_kv
        ...
        reserved = 0
        for other in self.requests.values():
            if other is owner or other.unit.req is None:
                continue
            r = other.unit.req
    ...
    def messages(self, rid, data, output=None, *, flush=False):
        ...
        if get_active_stage() != stages[-1]:
            return
        ref = self.requests[rid].ref
        hook = self.adapter.flush if flush else self.adapter.stream
        chunks = hook(ref, data) if flush else hook(ref, data, output)
        for chunk in chunks:
            encoded = asdict(chunk)
            unit.emitted_count += 1
            unit.emitted_bytes += wire_size(encoded)
            limits = self.metadata(unit.payload)["output_limits"]
            if (
                unit.emitted_count > limits["chunks"]
                or unit.emitted_bytes > limits["bytes"]
            ):
                raise QueueFullError()
    ...
    def usage(self, ref: SessionRef) -> ResourceUsage:
        ...
        owner = self.owners.get(sid)
        active = owner.unit.req if owner is not None and owner.unit else None
        kv = (
            active.kv
