...
class OmniScheduler:
    ...
    @staticmethod
    def _waiting_requests_allow_after_build_drain_hold(waiting: list[Any]) -> bool:
        hints = [
            getattr(
        ...
        explicit_hints = [hint for hint in hints if hint is not None]
        return not explicit_hints or any(hint is True for hint in explicit_hints)
    ...
    def get_new_batch_prefill(self, running_batch):
        ...
        if self.prefill_coalesce_requires_pending_builds:
            ...
            hold_after_builds = (
                ...
                and self._waiting_requests_allow_after_build_drain_hold(waiting)
            )
