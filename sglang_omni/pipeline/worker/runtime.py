...
class Worker:
    ...
    def __init__(self, executor: Executor, role: str | None = None):
        ...
        self._stream_send_queue: _queue_mod.Queue[_PendingStreamItem] = _queue_mod.Queue()
        ...
        self._same_gpu_targets: set[str] = set()  # Set by compiler for CUDA IPC zero-copy
        ...
        try:
            ...
            for target in self._stream_targets:
                try:
                    ...
                    logger.debug("Worker: failed to send stream done for %s to %s", request_id, target)
    ...
    def _enqueue_stream(self, request_id: str, data: Any, target_stage: str,
                        metadata: dict | None = None) -> None:
        """Non-blocking enqueue. Must not block — may be called from the event loop."""
        try:
            self._stream_send_queue.put_nowait(
                _PendingStreamItem(request_id=request_id, data=data,
                                   target_stage=target_stage, metadata=metadata),
            )
        except _queue_mod.Full:
            logger.error("Stream send queue full for %s → %s, dropping chunk",
                         request_id, target_stage)

    def _enqueue_stream_done(self, request_id: str, target_stage: str) -> None:
        """Non-blocking end-of-stream signal."""
        try:
            self._stream_send_queue.put_nowait(
                _PendingStreamItem(request_id=request_id, data=None,
                                   target_stage=target_stage, is_done=True),
            )
        except _queue_mod.Full:
            logger.error("Stream send queue full, dropping done signal for %s", request_id)

    def _enqueue_stream_error(self, request_id: str, target_stage: str, error: str) -> None:
        """Non-blocking error signal."""
        try:
            self._stream_send_queue.put_nowait(
                _PendingStreamItem(request_id=request_id, data=None,
                               target_stage=target_stage, error=error),
            )
        except _queue_mod.Full:
            logger.error("Stream send queue full, dropping error signal for %s", request_id)
    ...
    async def _stream_send_loop(self) -> None:
        ...
        while self._stream_running or not self._stream_send_queue.empty():
            ...
            try:
                ...
                logger.exception("Worker: failed to send stream item for %s", item.request_id)
    ...
    async def _do_stream_send(self, item: _PendingStreamItem) -> None:
        ...
        target_endpoint = self.stage.endpoints.get(item.target_stage) if self.stage else None
        ...
        # ── Cross-GPU: use relay ──
        blob_key = f"{item.request_id}:stream:{self.stage.name}:{item.target_stage}:{chunk_id}"
    ...
    async def _send_stream_control_message(self, msg, target_stage: str,
        ...
        if self.stage is None:
            ...
            return
        if endpoint is None:
            logger.warning("Worker: no endpoint for stage %s, dropping message", target_stage)
            return
    ...
    def _notify_stream_error(self, request_id: str, error: str) -> None:
        ...
        for target in self._stream_targets:
            try:
                ...
                logger.debug("Worker: failed to propagate stream error for %s", request_id)
