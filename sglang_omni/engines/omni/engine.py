            ...
            if queue is not None and not queue.empty():
                try:
                    ...
                    if isinstance(item, BaseException):
                        logger.error("Feedback exception for request %s: %s", req_id, item)
                        self.scheduler.fail_request(req_id, item if isinstance(item, Exception) else RuntimeError(str(item)))
                        continue
                    if isinstance(item, StreamSignal):
                        if item.error is not None:
                            logger.error("Feedback error for request %s: %s", req_id, item.error)
                            err = item.error if isinstance(item.error, Exception) else RuntimeError(str(item.error))
