...
def is_single_latin_word(
    ...
    return all(is_repeat_unit(char) for char in candidate) and all(
        char.isdigit()
...
def has_sustained_repetition(text: str) -> bool:
    """Detect 8-128 normalized units repeated at least three times."""
    normalized = unicodedata.normalize("NFKC", text).casefold()
    unit_offsets = tuple(
        index for index, char in enumerate(normalized) if is_repeat_unit(char)
    )
...
class WhisperTranscriptionAdapter(DefaultTranscriptionAdapter):
    ...
    def should_retry_chunk_without_context(self, text: str) -> bool:
        return has_sustained_repetition(text)
