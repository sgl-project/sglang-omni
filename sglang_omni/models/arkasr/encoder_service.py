...
Note (Akazaakane): PR #1411 made ARK's get_audio_feature pad and mask batches
across mixed mel lengths, so groups drained here now run as one fused
tower-and-adapter forward. When the opt-in encoder CUDA graph is enabled, this
service is also the main call site that replays captured graph buckets off the
LM scheduler path.
"""
