...
class Rope(NamedTuple):
    ...
    ``freqs`` and ``scale`` are what x_transformers returns. ``cos`` and ``sin``
    are the tables the fused Q/K kernel reads, and are set only where that
    kernel runs. They are built next to the rope, in ``AuKDit.forward``, rather
    than cached inside the fused path: the blocks compile, and a block that
    looked its tables up in a python dict would guard on keys that change with
    every trajectory, so it would recompile until dynamo gave up on it.
    """
...
class Attention(nn.Module):
    ...
    def _norm_rope(self, q, k, q_norm, k_norm, rope: Rope | None):
...
class AuKDit(nn.Module):
    ...
    def enable_fused_qk_norm_rope(self, fusion) -> None:
        """Route every block's Q/K norm and rope through ``fusion``.
    ...
    def _rope(self, seq_len: int, positions: torch.Tensor | None = None) -> Rope:
    ...
    def forward(
        ...
        seq_len = x.shape[1]
        text_len = c.shape[1]
        rope_audio = self._rope(seq_len, audio_positions)
        rope_text = self._rope(text_len)
        ...
        rope = self._rope(text_len + seq_len, joint_positions)
        for block in self.single_transformer_blocks:
