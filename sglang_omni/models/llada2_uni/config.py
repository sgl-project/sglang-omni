...
class LLaDA2UniInterleavedPipelineConfig(PipelineConfig):
    ...
    terminal_stages_fn: str | None = f"{_PKG}.routing.resolve_terminal_stages"
    total_gpu_memory_fraction_dict: ClassVar[dict[str, float]] = (
        LLaDA2UniOmniPipelineConfig.total_gpu_memory_fraction_dict
    )
        ...
        StageConfig(
            ...
            runtime=StageRuntimeConfig(
                ...
                sglang_server_args=SGLangServerArgsConfig(
                    mem_fraction_static=total_gpu_memory_fraction_dict[THINKER_STAGE]
                ),
            ),
            next=[THINKER_STAGE, IMAGE_DECODE_STAGE, INTERLEAVED_COLLECT_STAGE],
            route_fn=f"{_PKG}.routing.thinker_next",
            project_payload={
                THINKER_STAGE: f"{_PKG}.routing.project_interleaved_payload",
                IMAGE_DECODE_STAGE: f"{_PKG}.routing.project_interleaved_payload",
                INTERLEAVED_COLLECT_STAGE: (
                    f"{_PKG}.routing.project_interleaved_payload"
                ),
            },
        ),
        StageConfig(
