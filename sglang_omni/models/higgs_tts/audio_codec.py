...
def _device_graph_class(device_module: Any) -> Any:
    """Return the graph type this accelerator captures into (named per backend)."""
    for name in ("CUDAGraph", "XPUGraph"):
        graph_class = getattr(device_module, name, None)
        if graph_class is not None:
            return graph_class
    raise RuntimeError(
        f"{device_module.__name__} exposes no graph type, so the Higgs codec "
        "cannot capture decode graphs on this device"
    )
...
class HiggsAudioCodec:
    ...
    def _capture_decode_cuda_graphs_locked(self, frame_counts: tuple[int, ...]) -> None:
        ...
        graph_class = _device_graph_class(device_module)
