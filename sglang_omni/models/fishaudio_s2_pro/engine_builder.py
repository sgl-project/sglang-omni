...
def _resolve_fast_ar_attention_backend(
    *, gpu_id: int, device_type: str | None = None
) -> str:
    return _resolve_fast_ar_backend_plan(gpu_id=gpu_id, device_type=device_type).backend


def _resolve_fast_ar_backend_plan(*, gpu_id: int, device_type: str | None = None):
    return resolve_fast_ar_backend_plan(
        gpu_id=gpu_id,
        device_type=device_type,
        sm_resolver=get_visible_gpu_sm_version,
...
class FishS2ProEngineBuilder(TtsEngineBuilder):
    ...
    def generation_defaults(
        ...
        *,
        ...
        del dtype
        fast_ar_plan = _resolve_fast_ar_backend_plan(
            gpu_id=self.gpu_id, device_type=self._device_type()
        )
        return {
            "max_running_requests": 64,
            "disable_cuda_graph": not fast_ar_plan.enable_cuda_graph,
            "mem_fraction_static": 0.85,
            "chunked_prefill_size": 8192,
            "dtype": "bfloat16",
            "enable_torch_compile": fast_ar_plan.enable_torch_compile,
            "random_seed": int.from_bytes(os.urandom(4), "little") & 0x7FFFFFFF,
        }

    def adjust_overrides(self, overrides: dict[str, Any]) -> None:
        if overrides.get("attention_backend") is None:
            fast_ar_backend = _resolve_fast_ar_attention_backend(
                gpu_id=self.gpu_id, device_type=self._device_type()
            )
