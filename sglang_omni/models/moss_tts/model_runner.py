...
class MossTTSModelRunner(ModelRunner):
    ...
    def sample_tokens(
        ...
        *,
        ...
        use_host_sampler = device.type == "cpu" or current_platform.is_npu()
        if candidate_token_ids is not None:
            if use_host_sampler:
                candidate_sampled = multinomial_with_seed_host(
                    scores,
                    seeds_row,
                    positions_row,
                    candidate_token_ids,
        ...
        if bool(sample_mask.any()):
            if use_host_sampler:
                sampled[sample_mask] = multinomial_with_seed_host(
                    scores[sample_mask],
