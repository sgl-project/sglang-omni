...
# LOCAL MODIFICATION (sglang-omni): upstream called `logging.basicConfig(level=DEBUG)` at import,
# which reconfigures the process-wide root logger to DEBUG the moment the vendored frontend imports
# this module (flooding the server's logs). A vendored library module must not reconfigure the root
# logger, so the call is removed.
