...
from sglang_omni.models.cosyvoice3.cosyvoice.transformer.activation import Snake
from sglang_omni.models.cosyvoice3.cosyvoice.transformer.convolution import (
    CausalConv1d,
    CausalConv1dDownSample,
    CausalConv1dUpsample,
)
from sglang_omni.models.cosyvoice3.cosyvoice.utils.common import (
    get_padding,
    init_weights,
)
...
class CausalHiFTGenerator(HiFTGenerator):
    ...
    def inference(
        ...
        f0 = self.f0_predictor(speech_feat.to(torch.float64), finalize=finalize).to(
            speech_feat
        )
        # f0->source
        s = self.f0_upsamp(f0[:, None]).transpose(1, 2)  # bs,n,t
...
if __name__ == "__main__":
    ...
    for i in range(0, max_len, chunk_size):
        ...
        print(
            (pred_gt[:, i * 480 : i * 480 + pred_chunk.shape[1]] - pred_chunk)
            .abs()
            .max()
            .item()
        )
