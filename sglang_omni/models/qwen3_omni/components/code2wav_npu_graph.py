...
"""Exact-shape NPUGraph runner for the Qwen3-Omni Code2Wav component."""
...
class Code2WavNpuGraphRunner:
    """Exact-shape NPUGraph runner for ``[B, Q, T]`` long codes.

    One instance is permanently bound to one model, NPU device, quantizer
    ...
    def build(
        ...
        *,
        ...
        runner = cls(
            ...
            npu_api=_TorchNpuApi() if npu_api is None else npu_api,
        )
    ...
    def _build(self, total_gpu_memory_fraction: float | None) -> None:
        ...
        try:
            with self._npu.device_context(self._device):
                before = self._npu.memory_stats(self._device)
    ...
    def _capture_attempt(
        ...
        *,
        ...
        try:
            with self._npu.device_context(self._device):
                pool = self._npu.graph_pool_handle()
                capture_stream = self._npu.new_stream(self._device)
                if tier1_keys:
                ...
                for index, key in enumerate(tier1_keys):
                    ...
                    self._npu.synchronize(self._device)
                ...
                if violation_index is None:
                    ...
                    gc.collect()
                    self._npu.empty_cache()
                    after = self._npu.memory_stats(self._device)
    ...
    def _footprint_since(self, before: dict[str, int]) -> int:
        snapshot = self._npu.memory_stats(self._device)
        return max(
            0,
    ...
    def _capture_graph(
        ...
        *,
        ...
        static_input = self._npu.new_static_input(
        ...
        graph, static_output = self._npu.capture(
            self._model,
            static_input,
            pool=pool,
            stream=stream,
    ...
    def _rollback_build(
        ...
        *,
        ...
        if "after" not in self._memory_stats:
            ...
            try:
                self._memory_stats["after"] = self._npu.memory_stats(self._device)
        ...
        try:
            with self._npu.device_context(self._device):
                self._npu.empty_cache()
                self._memory_stats["after_rollback"] = self._npu.memory_stats(
                    self._device
                )
        except Exception as cleanup_exc:
            logger.warning(
    ...
    def _validate_codes(self, codes: torch.Tensor) -> None:
        if not self._npu.is_npu_tensor(codes):
            raise TypeError(
        ...
        if not self._npu.tensor_device_matches(codes, self._device):
...
__all__ = ["Code2WavNpuGraphRunner"]
