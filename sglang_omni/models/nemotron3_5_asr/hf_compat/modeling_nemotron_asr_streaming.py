...
class NemotronAsrStreamingEncoderCausalConv1dCacheLayer:
    ...
    def update(self, hidden_states, conv_module=None):
        ...
        if self.left_pad > 0:
            ...
            if shortfall > 0:
                padding_states = torch.cat(
                    [self.cache[:, :, -shortfall:], hidden_states], dim=-1
                )
            else:
    ...
    def update(self, hidden_states, cache_key, conv_module):
        if cache_key not in self.layers:
            if isinstance(conv_module, NemotronAsrStreamingEncoderCausalConv2D):
                self.layers[cache_key] = (
                    NemotronAsrStreamingEncoderCausalConv2dCacheLayer()
                )
            elif isinstance(conv_module, NemotronAsrStreamingEncoderCausalConv1d):
                self.layers[cache_key] = (
                    NemotronAsrStreamingEncoderCausalConv1dCacheLayer()
                )
            else:
...
class NemotronAsrStreamingEncoderCausalConv2D(nn.Conv2d):
    ...
    def output_length(
        self, input_lengths: torch.Tensor | None, streaming: bool = False
    ) -> torch.Tensor | None:
...
class NemotronAsrStreamingEncoderRelPositionalEncoding(nn.Module):
    ...
    def __init__(self, config: NemotronAsrStreamingEncoderConfig, device=None):
        ...
        self.config = config
        inv_freq = self.compute_default_relative_positional_parameters(
    ...
    def compute_default_relative_positional_parameters(
        ...
        inv_freq = 1.0 / (
            base
            ** (
                torch.arange(0, config.hidden_size, 2, dtype=torch.int64).to(
    ...
    def forward(self, hidden_states: torch.Tensor, cached_frames: int | None = None):
        ...
        seq_length = hidden_states.shape[1] + (
            cached_frames if cached_frames is not None else 0
        )
...
class NemotronAsrStreamingEncoderAttention(nn.Module):
    ...
    def __init__(self, config: NemotronAsrStreamingEncoderConfig, layer_idx: int):
        ...
        self.bias_u = nn.Parameter(
            torch.zeros(config.num_attention_heads, self.head_dim)
        )
    ...
    def _rel_shift(self, attention_scores):
        ...
        attention_scores = attention_scores.view(
            batch_size, num_heads, -1, query_length
        )
...
class NemotronAsrStreamingEncoderSubsamplingConv2D(nn.Module):
    def __init__(self, config: NemotronAsrStreamingEncoderConfig):
        ...
        self.layers = nn.ModuleList(
            NemotronAsrStreamingEncoderSubsamplingLayer(config, layer_idx=i)
            for i in range(1, num_layers)
        )
        self.act_fn = nn.ReLU()
        self.linear = nn.Linear(
            config.subsampling_out_hidden_size, config.hidden_size, bias=True
        )
    ...
    def forward(
        ...
        for layer in self.layers:
            hidden_states, lengths = layer(
                hidden_states, lengths, padding_cache=padding_cache
            )
...
class NemotronAsrStreamingEncoderBlock(GradientCheckpointingLayer):
    def __init__(
        self, config: NemotronAsrStreamingEncoderConfig, layer_idx: int | None = None
    ):
        super().__init__()
        ...
        self.conv = NemotronAsrStreamingEncoderConvolutionModule(
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        hidden_states = (
            residual + 0.5 * hidden_states
        )  # the conformer architecture uses a factor of 0.5
...
class NemotronAsrStreamingPreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, NemotronAsrStreamingEncoderAttention):
            ...
            buffer_value = module.compute_default_relative_positional_parameters(
                module.config
            )
    ...
    def _get_output_attention_mask(
        ...
        attention_mask = (
            torch.arange(max_length, device=attention_mask.device)
            < output_lengths[:, None]
        )
...
class NemotronAsrStreamingEncoder(NemotronAsrStreamingPreTrainedModel):
    ...
    def __init__(self, config: NemotronAsrStreamingEncoderConfig):
        ...
        self.layers = nn.ModuleList(
            [
                NemotronAsrStreamingEncoderBlock(config, layer_idx)
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        position_embeddings = self.encode_positions(
            inputs_embeds, cached_frames=cached_frames
        )
        ...
        return NemotronAsrStreamingEncoderModelOutput(
            ...
            attention_mask=(
                output_mask.int()
                if output_mask is not None and output_attention_mask
                else None
            ),
            past_key_values=past_key_values,
            padding_cache=padding_cache,
