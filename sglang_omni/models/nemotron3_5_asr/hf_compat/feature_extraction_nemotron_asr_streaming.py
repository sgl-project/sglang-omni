    ...
    def __call__(
        ...
        **kwargs,
        ...
        batched_speech = BatchFeature(
            {"input_features": raw_speech, "audio_lengths": audio_lengths}
        )
        ...
        if self.preemphasis is not None:
            ...
            ).unsqueeze(0) < padded_inputs.audio_lengths.unsqueeze(1)
        ...
        if center:
            ...
            features_lengths = torch.floor_divide(
                padded_inputs.audio_lengths + self.n_fft // 2 * 2 - self.n_fft,
                self.hop_length,
            )
        else:
            ...
            features_lengths = (
                torch.floor_divide(
                    padded_inputs.audio_lengths - self.n_fft, self.hop_length
                )
                + 1
            )
        attention_mask = (
            torch.arange(input_features.shape[1], device=device)[None, :]
