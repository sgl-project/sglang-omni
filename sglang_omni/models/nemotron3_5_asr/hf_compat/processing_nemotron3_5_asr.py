...
class Nemotron3_5AsrProcessor(ProcessorMixin):
    ...
    def _get_arguments_from_pretrained(
        ...
        **kwargs,
        ...
        """Load the 5.13 nested feature-extractor config on Transformers 5.12.

        The Nemotron 3.5 checkpoint stores its feature extractor under the
        ``feature_extractor`` key in ``processor_config.json``. Transformers
        5.12 treats the primary feature extractor as an AutoFeatureExtractor
        lookup instead, but its auto mapping predates this model. Constructing
        the two known sub-processors explicitly keeps this backport local and
        avoids mutating process-wide Auto mappings.
        """
        processor_dict = processor_dict or {}
        feature_extractor_config = processor_dict.get("feature_extractor")
        ...
        tokenizer = ParakeetTokenizer.from_pretrained(
            pretrained_model_name_or_path,
            subfolder=subfolder,
            **kwargs,
        )
    ...
    def __init__(
        ...
        self.prompt_dictionary = (
            ...
            else DEFAULT_PROMPT_DICTIONARY
        )
    ...
    def model_input_names(self):
        ...
        return feature_extractor_input_names + [
            "labels",
            "decoder_input_ids",
            "prompt_ids",
        ]
