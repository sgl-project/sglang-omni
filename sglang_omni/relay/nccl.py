...
from .base import Relay, RelayOperation, CreditAllocator, register_relay
...
class Connection:
    ...
    def __init__(
        ...
        if any(r >= world_size or r < 0 for r in send_ranks + recv_ranks):
            raise ValueError(f"Invalid rank in topology: send={send_ranks}, recv={recv_ranks}, world_size={world_size}")
        ...
        logger.info(f"[{engine_id}] Connection initialized. Rank: {rank}, Send->{send_ranks}, Recv<-{recv_ranks}")

    def get_agent_metadata(self) -> Dict:
        return {"rank": self.rank, "engine_id": self.name}

    def ensure_remote_agent(
        self, remote_engine_id: str, remote_meta_bytes: Any
    ) -> int:
        target_rank = remote_meta_bytes.get("rank", 0)
        if target_rank not in self.recv_ranks:
            logger.warning(f"[{self.name}] Receiving data from rank {target_rank} which is NOT in recv_ranks {self.recv_ranks}!")
        return target_rank
...
class NcclOperation(RelayOperation):
    ...
    """
    def __init__(self, connection: Connection, work_handle, tensor_ref: Any, metadata: Any = None):
...
class NcclRelay(Relay):
    def __init__(
        ...
        logger.info(f"[{engine_id}] Initialized NCCL Relay on {device} (Rank {rank}). Starting Warmup...")
        ...
        try:
            ...
            logger.error(f"[{engine_id}] NCCL Warmup failed! Check topology consistency.")
            raise e
    ...
    async def put_async(
        ...
        if dst_rank not in self.connection.send_ranks:
            logger.warning(f"Sending to rank {dst_rank} which is NOT in send_ranks whitelist!")
    ...
    async def get_async(
        self, metadata: Any, dest_tensor: torch.Tensor, request_id: str = None, src_rank: int = None
    ) -> GetOperation:
        ...
        if src_rank is None:
            src_rank = self.connection.ensure_remote_agent(remote_engine_id, remote_agent_meta)
