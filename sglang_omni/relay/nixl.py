...
try:
    ...
    class NixlAgent:
        ...
        def get_agent_metadata(self): return b'meta'
        def add_remote_agent(self, *args): return 'agent'
        def register_memory(self, *args): return 1
        def deregister_memory(self, *args): pass
        def check_xfer_state(self, *args): return "DONE"
        def release_xfer_handle(self, *args): pass
        def get_new_notifs(self): return {}
        def get_xfer_descs(self, *args): return []
        def prep_xfer_dlist(self, *args): return 0
        def make_prepped_xfer(self, *args, **kwargs): return 0
        def transfer(self, *args): pass
    def nixl_agent_config(**kwargs): return None
        ...
        try:
            while True:
                ...
                if time.time() - start > timeout:
                    raise TimeoutError(f"PutOperation timed out waiting for {self._expected_notification}")
...
class GetOperation(NixlOperation):
    ...
    def __init__(self, 
                 ...
                 copy_size: int,
                 on_completion_cb: Callable[[], None]):
        super().__init__(connection, metadata=None) # Get usually doesn't return metadata
        self._handle = handle
...
class NixlRelay:
    def __init__(self, engine_id: str, slot_size_mb: int = 64, credits: int = 2, device: str = "cuda"):
        ...
        self.pool_tensor = torch.zeros(total_pool_bytes, dtype=torch.uint8, device=device)
    ...
    async def put_async(self, tensor: torch.Tensor, request_id: str = None) -> PutOperation:
        """
    ...
    async def get_async(self, metadata: Any, dest_tensor: torch.Tensor, request_id: str = None) -> GetOperation:
        """
