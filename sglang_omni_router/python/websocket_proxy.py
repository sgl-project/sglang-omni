...
class TTSWebSocketProxy:
    ...
    async def forward(self, websocket: WebSocket) -> None:
        ...
        session_request_id = request_id(websocket)
        ...
        try:
            if not is_admitted:
                ...
                await self.complete_session(websocket, None, result=result)
                return

            first_message = await self.receive_initial_message(websocket)
            if isinstance(first_message, SessionResult):
                result = first_message
                await self.complete_session(websocket, None, result=result)
                return

            facts = session_route_facts(first_message)
            try:
                worker = self.select_worker(facts)
            ...
            with worker.request_guard():
                result = await self.connect_and_relay(
                    websocket,
                    worker,
                    first_message,
                )
            ...
            result = SessionResult(outcome="client_disconnected")
            await self.complete_session(
                websocket,
                worker,
                result=result,
            ...
            if result is None:
                result = SessionResult(outcome="session_cancelled")
            raise
        finally:
            if is_admitted:
    ...
    async def receive_initial_message(
        ...
        if message.get("type") == "websocket.disconnect":
            return SessionResult(outcome="client_disconnected")
        message_limit = min(
            self._config.max_payload_size,
            MAX_SPEECH_WS_CONFIG_MESSAGE_BYTES,
        )
        if message_size(message) <= message_limit:
            return message
    ...
    async def connect_and_relay(
        ...
        try:
            async with websocket_connect(
                upstream_url(worker, websocket),
                **connect_options,
            ) as upstream:
                await send_upstream(upstream, first_message)
                outcome = await relay(
                    websocket,
                    upstream,
            ...
            return connection_failure_result(exc)
        return relay_result(outcome)
    ...
    def select_worker(self, facts: SpeechRouteFacts) -> Worker:
...
def relay_result(outcome: RelayOutcome) -> SessionResult:
    if outcome == "completed":
        return SessionResult(outcome=outcome, routed_status_code=200)
...
def connection_failure_result(exc: Exception) -> SessionResult:
    status_code = handshake_status_code(exc)
    if status_code is not None:
        failure = handshake_failure(status_code)
    ...
    if isinstance(exc, ConnectionClosed) and is_application_close(exc):
        return SessionResult(
...
async def relay(
    ...
    *,
    ...
    upstream_task = asyncio.create_task(upstream_to_client(upstream, websocket))
    try:
        return await coordinate_relay(
            websocket,
            upstream,
        ...
        await cancel_relay_tasks(client_task, upstream_task)
...
async def coordinate_relay(
    ...
    *,
    ...
    raise_completed_task_error(done)
    if client_task in done:
        ...
        if client_outcome == "client_disconnected":
            ...
            await close_upstream(upstream, code=1009)
    ...
    if outcome == "client_disconnected":
        await close_upstream(upstream, code=1000)
    else:
        client_task.cancel()
        ...
        with suppress(RuntimeError, WebSocketDisconnect):
            await websocket.close(code=safe_close_code(code), reason=reason)
    return outcome
...
async def client_to_upstream(
    ...
    *,
    ...
    while True:
        ...
        try:
            await send_upstream(upstream, message)
    ...
    try:
        async for message in upstream:
            if isinstance(message, str):
                ...
                if not await send_downstream(websocket.send_text(message)):
                    return "client_disconnected"
            else:
                if not await send_downstream(websocket.send_bytes(bytes(message))):
                    return "client_disconnected"
    except ConnectionClosed as exc:
        if protocol.has_application_failure or is_application_close(exc):
...
async def send_downstream(send: Awaitable[None]) -> bool:
    try:
        await send
    except (OSError, RuntimeError, WebSocketDisconnect):
        return False
...
def handshake_status_code(exc: Exception) -> int | None:
...
def handshake_failure(status_code: int) -> HandshakeFailure:
...
def is_application_close(exc: ConnectionClosed) -> bool:
...
def upstream_url(worker: Worker, websocket: WebSocket) -> str:
...
def forward_headers(websocket: WebSocket) -> dict[str, str]:
    return {
        key: value
...
def request_id(websocket: WebSocket) -> str:
    return (
...
def safe_close_code(code: int) -> int:
